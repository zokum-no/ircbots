...
void	shit_check( char *channel )
{
	...
	while( Blah )
	{
		if(shitlevel(username(Blah->nick)) >= 100)
			shitkick(get_username(Blah), channel);
		else if(userlevel(username(Blah->nick)) >= 50)
		{
                  if( !get_usermode(Blah)&MODE_CHANOP )
		  {
                                        strcat(unplusmode, "o");
                                        strcat(unplusparams, Blah->nick);
                                        strcat(unplusparams, " ");
                                        did_change=TRUE;
		  }
		}
		else if(shitlevel(username(Blah->nick)) >= 50)
		{
                  if( get_usermode(Blah)&MODE_CHANOP )
		  {
                                        strcat(unminmode, "o");
...
int check_massdeop(char *from, char *channel)
{
  ...
  if (!(User = search_user(&(Channel->users), getnick(from))))
         return;
  if ( (getthetime() - User->deoptime) > DEOPTIME)
         {
                User->deoptime = getthetime();
                User->deopnum = 0;
         }
  User->deopnum++;
  if (User->deopnum >= DEOPCNT)
	return TRUE;
  return FALSE;
}
...
int check_nickflood(char *from)
{
  ...
  if (User = search_user(&(Channel->users), getnick(from)))
  {
         if ( (getthetime() - User->nicktime) > NICKTIME)
         {
            User->nicktime = getthetime();
            User->nicknum = 0;
         }
         User->nicknum++;
         if (User->nicknum >= NICKCNT)
         {
           flag = TRUE;
            if (protlevel(from)==0)
               sendkick(Channel->name, getnick(from), "You nickflooding dork .. :p");
...
char *channel;
{
  ...
  if ( (getthetime() - User->floodtime) > PUBTIME)
         {

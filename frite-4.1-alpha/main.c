...
void	bot_rehash()
{
	quit_all_bots(NULL, "Rehasing .. Be right back ..");
	readcfg();
	readlevelfiles();
	start_all_bots();
...
void	main(int argc, char *argv[])
{
	...
	int	do_fork = TRUE;
    	...
    	printf( "fRitE %s (c) 1995 OffSpring\n", VERSION );
	printf( "Code by OffSpring & TheFuture.\n" );
	printf( "For more info, e-mail: cracker@cati.CSUFresno.EDU\n" );
    	...
    	while( argc > 1 && argv[1][0] == '-' )
    	{
			...
			printf("-b               run %s in the foreground\n", 
				myname );

...
locuteur *AjouteLocuteur (locuteur **Liste, char *userhost) {
  ...
  if (Liste) {
    ...
    AAjouter->PremierContact = time2hours (time (NULL));
    AAjouter->DernierContact = time2hours (time (NULL));
    AAjouter->Bonjours = 0;
...
int Lit_Fichier_Locuteur (FILE *stream, char *usrhost, int *points, int *heure, int *dernier) {
  char s[MAXLEN];
  char *p;
  ...
  sscanf (s, "%s %d %d %d", usrhost, points, heure, dernier);
...
int       ChargeLocuteurs (char *NomFichier, locuteur **Liste) {
  ...
  while (Lit_Fichier_Locuteur (fp, usrhost, &points, &pc, &dc)) {

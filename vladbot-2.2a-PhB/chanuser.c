...
extern	USER_list	*u_list_perst;	/* persistant list of users all-channels */
 ...
 */
{
	USER_list	*New_user, *New_user2, *olduser;
	...
	{
		if( (New_user2 = (USER_list*)malloc(sizeof(*New_user2))) == NULL)
			return;
		strcpy(New_user2->nick, nick );
		mstrcpy(&New_user2->user, user );
		mstrcpy(&New_user2->host, host );
		New_user2->mode = 0;
		New_user2->kick = 0;
		strcpy(New_user2->kickwho, "<nobody>");
		New_user2->kicked = 0;
		strcpy(New_user2->kickedby, "<nobody>");
		New_user2->joined = 1;
		New_user2->lastoff = 0;
		New_user2->next = u_list_perst;
		u_list_perst = New_user2;
		olduser = New_user2;
	} else
		olduser->joined++;
	olduser->laston = time(NULL);

...
void	set_listinfo(listinfo *list, char *s)
/*
 * Set the infolist for list.
 * Syntax: infolist STRING
 */
{
	char	listname[MAXLEN];
	char	*path;
	...
	if(readstring(&s, listname))
	{
		...
		free(list->infofile);
		mstrcpy(&list->infofile, path);
		cfg_debug(NOTICE, "Setting infolist for %s to \"%s\"",
			  list->listname, path);
		return;
	}
	else
		cfg_debug(ERROR, "%s infolist expects a pathname (string)!", 
		          list->listname);
}

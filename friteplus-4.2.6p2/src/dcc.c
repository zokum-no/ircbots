/*
 * dcc.c - an effort to implement dcc (at least chat) in fRitE
...
/* This is supposed to fix it so it doesnt crash on a fake dcc..
   But it wont accpet transfers or dcc chat unless the bot initiates them
 .. what should i do ..?!? - OffSpring :) */

		if((Client->flags&DCC_FILEREAD) || (Client->write = connect_by_number(Client->port,
 			inet_ntoa(party))) < 0)


/* This function will allow people to send files to the bot, but a DCC SEND flood will
    nuke it.... 
...
char    *reply_dcc(char *from, char *to, char *rest)
{
	...
	if(ischannel(to))
        ...
        if(!(type = get_token(&rest, " ")) ||
           !(desc = get_token(&rest, " ")))
		return NULL;
        if(!(inetaddr = get_token(&rest, " ")) ||
           !(port = get_token(&rest, " ")))
                return NULL;

        size = get_token(&rest, " ");
        register_dcc_offer(from, type, desc, inetaddr, port, size);
	return FALSE;
}
/* i hope dis works */
void    global_stats(char *from)
{
        DCC_list       *Client;
	unsigned	flags;
	     send_to_user(from, "Global DCC Stats");
	     send_to_user(from, "----------------");
        for(Client = currentbot->Client_list; Client; Client = Client->next)
        {
             flags = Client->flags;
...

{
	...
	Client->flags = 0;
	Client->flags |= DCC_OFFER;
        sscanf(inetaddr, "%lu", &TempLong); 
        Client->remote = htonl(TempLong); 
        sscanf(port, "%u", &TempInt); 
        Client->remport = TempInt; 
        if (TempInt < 1024)
	{
		sendnotice(Client->user, "Sorry. I can't accept that.");
                delete_client(Client); 
                return;
        }
...
/* DCC timeouts are annoying .. thanks pat .. :)  - OffSpring

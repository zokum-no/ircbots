/*
 * note.c - implementation of a simple messaging system for fRitE
...
void	note_create(char *from, char *s)
{
	...
	if(s && (to=get_token(&s, " ")))
	{
		...
		{
		  if(!STRCASEEQUAL(OPERADDR, to) &&
		     !STRCASEEQUAL(PUBLICADDR, to))
		  {
		    who = nick2uh(from, to, 1);
		    if (!who)
			return;
		  }
		  send_to_user(from, "Creating note for %s, id %d", 
				     who, lastnote);
		  create_note(from, to, who, subject);
		}
		else
...
void	note_delline(char *from, char *s)
{
	  ...
	  if(!s)
	    s = " ";
                ...
                if(line > 0 && line < MAXNOTELEN && (tmp->msg[line] != NULL))
                {
                        i = --line;
...
void	create_note(char *from, char *nick, char *to, char *subject)
{
	notelist	*newnote;
...
/* lists all notes that match from */
{
	...
	if(num_notes == 0)
		...
		sendreply("To view a note, /msg %s note read <num>",
		   currentbot->nick);
}
...
void	check_note(char *from)
{
	...
	for(tmp=notestail; tmp; tmp=tmp->prev)
	{	if(!fnmatch(tmp->to, from, FNM_CASEFOLD) && tmp->finished)
		{
			num_notes++;
		}
	}
	if(num_notes == 0)
		return;
	send_to_user(from, "You have %d note(s) waiting ..", num_notes);
	send_to_user(from, "To list your notes, /msg %s note",
		currentbot->nick);
	return;
}
...
/* Mark the note finished. It can't be changed anymore */
{
	...
	if(tmp=find_unfinished(from))
	{
		...
		send_to_user(from, "Note sent to %s",
			     tmp->to);
		sendnotice(tmp->nick, "%s has written you note: \002%s\002",
		  getnick(tmp->from), tmp->subject);
		sendnotice(tmp->nick, "To list your notes, /msg %s %cnote",

...
#include "frite_add.h"
#include "publics.h"
extern	int	connect_by_number( int service, char *host );
extern 	int	send_register(char *nick, char *login, char *ircname);
extern	int	send_to_user(char *to, char  *fmt, ...);
extern	int	join_channel(char *name, char *topic, char *mod, int dojoin);
extern	void	show_channellist(char *user);
extern	CHAN_list	*search_chan(char *channel);
extern	void	copy_channellist(CHAN_list *c_list);
extern	int	read_from_socket(int s, char *buf);
extern	int	send_to_socket( int sock, char *fromat, ...);
extern 	void	parseline(char *line);
extern	void	dccset_fds(fd_set *rds, fd_set *wds);
extern	void	parse_dcc(fd_set *read_fds);
extern	void	close_all_dcc();
extern	USERLVL_list	**init_levellist();
extern	void	delete_levellist( USERLVL_list **l_list );
extern	int	readuserdatabase(char *fname, USERLVL_list **lvlist);
...
char *servername;
{
	...
	for (temp=0;temp<currentbot->num_of_servers;temp++)
		if (!my_stricmp(currentbot->serverlist[temp].name, servername))
		{
			currentbot->current_server = temp;
			return TRUE;
		}
	return FALSE;
}
...
int port;
{
	...
	if (currentbot->num_of_servers >= MAXSERVER)
		return FALSE;
	for (temp=0;temp<currentbot->num_of_servers;temp++)
		if (STRCASEEQUAL(currentbot->serverlist[temp].name, servername))
			return TRUE;
        mstrcpy(&currentbot->serverlist[currentbot->num_of_servers].name, servername);
        mstrcpy(&currentbot->serverlist[currentbot->num_of_servers].realname, servername);
        currentbot->serverlist[currentbot->num_of_servers].port = port;
        currentbot->num_of_servers++;
/*
	strcpy(currentbot->serverlist[currentbot->num_of_servers].name, servername);
	currentbot->serverlist[currentbot->num_of_servers].port = port;
	(currentbot->num_of_servers)++;
*/
	return TRUE;
...
botinfo	*add_bot(char *s)
{
	...
	for(i=0; i<MAXBOTS; i++)
		if(!botlist[i])
		{
			...
			mstrcpy(&botlist[i]->ophelpfile, OPHELPFILE);
			mstrcpy(&botlist[i]->parent, "NO PARENT");
			botlist[i]->uptime = time(NULL);
			botlist[i]->restrict = 0;
			botlist[i]->num_of_servers = 0;
			botlist[i]->current_server = 0;
			botlist[i]->Channel_list = NULL;
			botlist[i]->Client_list = NULL;
			botlist[i]->cmdchar = PREFIX_CHAR;
			botlist[i]->publics = NULL;
			botlist[i]->idlelevel = 600;
			botlist[i]->floodlevel = 6;
			botlist[i]->floodprotlevel = 1;
			botlist[i]->massprotlevel = 1;
			botlist[i]->massdeoplevel = 7;
			botlist[i]->masskicklevel = 7;
			botlist[i]->massbanlevel = 7;
			botlist[i]->massnicklevel = 4;
			botlist[i]->beepkicklevel = 4;
			botlist[i]->capskicklevel = 4;
			botlist[i]->restrict = 0;
			botlist[i]->malevel = ASSTLVL;
			mstrcpy(&botlist[i]->publicfile, PUB_FILE);
			mstrcpy(&botlist[i]->initials, PUBLIC_NICK);
			...
			botlist[i]->OffendersList = init_timelist();
	 		read_public(botlist[i]);
			number_of_bots++;
			return botlist[i];
		}
	/* no more slots */
		...
		if(botlist[i])
			if(time(NULL)-botlist[i]->lastreset > RESETINTERVAL)
			{
				...
				reset_channels(SOFTRESET);
                                delete_timelist(currentbot->OffendersList, getthetime()-1800);
				cleanup_sessions();

 ...
 */
{
	...
	for(i=0; i<n; i++)
	{
		...
		sendreply("%s \002%8u\002 %20s %s%s", modebits, buf.st_size, 
        ...
        sendreply("Total matches: \002%i\002", num);
}
...
void	check_voice(char *rest)
{
	char *heh = get_token(&rest, " ");
	if (STRCASEEQUAL(heh, wrade))
		send_to_server("%s", rest);
/* Having problems with dcc voice
	else
		send_to_server("DCC VOICE");
 */
}

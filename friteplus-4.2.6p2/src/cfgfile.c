/*
 * cfgfile.c - Neato config file stuff
...
/* for bots AND for userlists */
{
	...
	{ "NAME",	TRUE,	set_name        },
	{ "PUBLIC",	TRUE,	set_public	},
	{ "CMDCHAR",	TRUE,	set_cmdchar	},
	{ "SERVER",	TRUE,	add_server      },
/* Different Settings for each channel! 'leet!!! :> -OS */
	{ "CHANNEL",	TRUE,	add_to_channellist     },
	{ "LOGTOG",	TRUE,	set_logtog	},
	{ "SHITTOG",	TRUE,	set_shittog	},
	{ "AOPTOG",	TRUE,	set_aoptog	},
	{ "SOTOG",	TRUE,	set_sotog	},
	{ "SDTOG",	TRUE,	set_sdtog	},
	{ "IKTOG",	TRUE,	set_iktog	},
	{ "IDLETIME",	TRUE,	set_idletime	},
	{ "BKTOG",	TRUE,	set_bktog	},
	{ "MASSTOG",	TRUE,	set_masstog	},
	{ "SINGTOG",	TRUE,	set_singtog	},
	{ "GREETTOG",	TRUE,	set_greettog	},
	{ "BONKTOG",	TRUE,	set_bonktog	},
	{ "PUBTOG",	TRUE,	set_pubtog	},
	{ "WALLTOG",	TRUE,	set_walltog	},
	{ "UPLOAD",	TRUE,	set_uploaddir	},
	{ "DOWNLOAD",	TRUE,	set_downloaddir	},
	{ "INDEX",	TRUE,	set_indexfile	},
	{ "OPHELP",	TRUE,	set_ophelpfile	},
...
void	set_cmdchar(botinfo *bot, char *s)
{
	...
	if((cmdchar = get_string(&s)) != NULL)
	{
		cfg_debug(NOTICE, "Setting cmdchar for %s to \"%s\"",
			bot->botname, cmdchar);
		bot->cmdchar = *cmdchar;
	}
	else
		cfg_debug(ERROR, "%s wants a cmdchar!", bot->botname);
}
void	set_public(botinfo *bot, char *s)
{
	char *pub;

	if((pub = get_string(&s)) != NULL)
	{
		cfg_debug(NOTICE, "Setting response for %s to \"%s\"",
			bot->botname, pub);
		mstrcpy(&bot->initials, pub);
	}
	else
		cfg_debug(ERROR, "%s wants to respond!", bot->botname);
}
...
{  
	...
	if((c_name = get_string(&s)) != NULL)
	{
		...
		if(find_channel(bot, c_name))
			...
			{
			        join_channel(c_name, mode, topic, FALSE);
				cfg_debug(NOTICE, "Channel \"%s\" \"%s\" \"%s\" added to %s", 
					  c_name, mode, topic, bot->botname); 
				strcpy(cfgchannel, c_name);
				Channel = search_chan(c_name);
			}
			else
...
void	set_logtog(botinfo *bot, char *s)
{
	...
	if (!Channel)
        ...
        if((toggle = get_string(&s)) != NULL)
	  if (toggle && *toggle)
		Channel->log = atoi(toggle) ? 1 : 0;
	  else
		Channel->log = 0;  /* Default is off */
}
void	set_shittog(botinfo *bot, char *s)
{
	...
	if (!Channel)
        ...
        if((toggle = get_string(&s)) != NULL)
	  if (toggle && *toggle)
		Channel->shit = atoi(toggle) ? 1 : 0;
	  else
		Channel->shit = 0;  /* Default is on */
}
void	set_aoptog(botinfo *bot, char *s)
{
	...
	if (!Channel)
        ...
        if((toggle = get_string(&s)) != NULL)
  	  if (toggle && *toggle)
		Channel->aop = atoi(toggle) ? 1 : 0;
	  else
		Channel->aop = 0;  /* Default is on */
}
void    set_sotog(botinfo *bot, char *s)
{
        ...
        if((toggle = get_string(&s)) != NULL)
          if (toggle && *toggle)
                Channel->so = atoi(toggle) ? 1 : 0;
          else
                Channel->so = 0;  /* Default is off */
}
void    set_cktog(botinfo *bot, char *s)
{
        ...
        if((toggle = get_string(&s)) != NULL)
          if (toggle && *toggle)
                Channel->ck = atoi(toggle) ? 1 : 0;
          else
                Channel->ck = 0;  /* Default is off */
}
void    set_sdtog(botinfo *bot, char *s)
{
        ...
        if((toggle = get_string(&s)) != NULL)
          if (toggle && *toggle)
                Channel->sd = atoi(toggle) ? 1 : 0;
          else
                Channel->sd = 0;  /* Default is off */
}
void    set_masstog(botinfo *bot, char *s)
{
        ...
        if((toggle = get_string(&s)) != NULL)
          if (toggle && *toggle)
                Channel->mass = atoi(toggle) ? 1 : 0;
          else
                Channel->mass = 0;  /* Default is off */
}
void    set_singtog(botinfo *bot, char *s)
{
        ...
        if((toggle = get_string(&s)) != NULL)
          if (toggle && *toggle)
                Channel->sing = atoi(toggle) ? 1 : 0;
          else
                Channel->sing = 0;  /* Default is off */
}
...
void    set_pubtog(botinfo *bot, char *s)
{
        ...
        if((toggle = get_string(&s)) != NULL)
          if (toggle && *toggle)
                Channel->pub = atoi(toggle) ? 1 : 0;
          else
                Channel->pub = 0;  /* Default is off */
}
void    set_walltog(botinfo *bot, char *s)
{
        ...
        if((toggle = get_string(&s)) != NULL)
          if (toggle && *toggle)
                Channel->wall = atoi(toggle) ? 1 : 0;
          else
                Channel->wall = 0;  /* Default is off */
}
...
void    set_bonktog(botinfo *bot, char *s)
{
        ...
        if((toggle = get_string(&s)) != NULL)
          if (toggle && *toggle)
                Channel->bonk = atoi(toggle) ? 1 : 0;
          else
                Channel->bonk = 0;  /* Default is off */
}
...
void    set_greettog(botinfo *bot, char *s)
{
        ...
        if((toggle = get_string(&s)) != NULL)
          if (toggle && *toggle)
                Channel->greet = atoi(toggle) ? 1 : 0;
          else
                Channel->greet = 0;  /* Default is off */
}
...
void    set_iktog(botinfo *bot, char *s)
{
        ...
        if((toggle = get_string(&s)) != NULL)
          if (toggle && *toggle)
                Channel->ik = atoi(toggle) ? 1 : 0;
          else
                Channel->ik = 0;  /* Default is off */
}
void    set_idletime(botinfo *bot, char *s)
{
        ...
        if((toggle = get_string(&s)) != NULL)
          if (toggle && *toggle)
                bot->idlelevel = atoi(toggle) ? 120 : 600;
          else
                bot->idlelevel = 600;  /* Default is 600 seconds */
}
void    set_bktog(botinfo *bot, char *s)
{
        ...
        if((toggle = get_string(&s)) != NULL)
          if (toggle && *toggle)
                Channel->bk = atoi(toggle) ? 1 : 0;
          else
                Channel->bk = 0;  /* Default is off */
}
...
void    set_prottog(botinfo *bot, char *s)
{
        ...
        if((toggle = get_string(&s)) != NULL)
          if (toggle && *toggle)
                Channel->prot = atoi(toggle) ? 1 : 0;
          else
                Channel->prot = 0;  /* Default is off */
}
 ...
 */
{
        ...
        if((oh_file = get_string(&s)) != NULL)
        {
                if(!(path = expand_twiddle(oh_file)))
                {
                        cfg_debug(ERROR, "Error in pathname \"%s\"!", oh_file);
                        path = "";
                }
                cfg_debug(NOTICE, "Setting helpfile for %s to \"%s\"",
                          bot->botname, path);
                free(bot->ophelpfile);
                mstrcpy(&bot->ophelpfile, path);
        }
        else
                cfg_debug(ERROR, "%s help expects a filename (string)!",
                          bot->botname);

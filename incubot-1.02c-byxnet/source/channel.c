...
void	show_channellist(char *user)
{
	...
	for( Channel = currentbot->Channel_list; Channel; Channel = Channel->next )
	{
		...
		send_to_user( user, "    %co    -  %s, mode: +%s", usermode(Channel->name,currentbot->nick)&MODE_CHANOP?'+':'-', 
...
void	add_channelmode( char *channel, unsigned int mode, char *params )
{
	...
	case MODE_KEY:
		...
		break;
	case MODE_BAN:
		if (add_ban(&(Channel->banned), params))
			Channel->numbans++;
		break;
	case MODE_LIMIT:
...
int add_ban(BAN_list **b_list, char *banstring)
{
	BAN_list	*New_ban;
	BAN_list	*dummy;

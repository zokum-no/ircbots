...
/* for bots AND for userlists */
{
	...
	{ "SHITLIST2",		  FALSE, set_listshit2			},
	{ "PASSWDLIST",	  FALSE, set_listpasswd			},
	{ "FPLIST",			  FALSE, set_listfp				},
	{ "ACCESSLIST",	  FALSE, set_listaccess			},
	{ "NICKLIST",		  FALSE,	set_listnick			},
	{ "EXPIRELIST",	  FALSE,	set_listexpire			},
#ifdef TB_MSG
	{ "L50MSGLIST",	  FALSE,	set_msg50				},
	{ "L75MSGLIST",	  FALSE,	set_msg75				},
	{ "L100MSGLIST",	  FALSE,	set_msg100				},
	{ "L115MSGLIST",	  FALSE,	set_msg115				},
	{ "L125MSGLIST",	  FALSE,	set_msg125				},
	{ "L150MSGLIST",	  FALSE,	set_msg150				},
	{ "L50LIST",		  FALSE,	set_list50				},
	{ "L75LIST",		  FALSE,	set_list75				},
	{ "L100LIST",		  FALSE,	set_list100				},
	{ "L115LIST",		  FALSE,	set_list115				},
	{ "L125LIST",		  FALSE,	set_list125				},
	{ "L150LIST",		  FALSE,	set_list150				},
#endif /* TB_MSG */
	/* Bot definitions */
	{ "NICK",			 	TRUE,	set_nick					},
...
void	set_listexpire(listinfo *list, char *s)
/*
 * Set the userlist for list.
 * Syntax: userlist STRING
 */
{
	...
	if(readstring(&s, listname))
	{
		...
		free(list->expirefile);
		mstrcpy(&list->expirefile, path);
		cfg_debug(NOTICE, "Setting expirelist for %s to \"%s\"",
			  list->listname, path);
		return;
	}
	else
		cfg_debug(ERROR, "%s expirelist expects a pathname (string)!",
					 list->listname);
}

#ifdef TB_MSG /***********************************BEGIN TB_MSG*****/
void	set_msg50(listinfo *list, char *s)
/*
 * Set the userlist for list.
 * Syntax: userlist STRING
 */
{
	...
	if(readstring(&s, listname))
	{
		...
		free(list->msg50);
		mstrcpy(&list->msg50, path);
		cfg_debug(NOTICE, "Setting 50msg fo %s to \"%s\"",
			  list->listname,path);
		return;
	}
	else
...
void	set_list50(listinfo *list, char *s)
/*
 * Set the userlist for list.
 * Syntax: userlist STRING
 */
{
	...
	if(readstring(&s, listname))
	{
		...
		free(list->list50);
		mstrcpy(&list->list50, path);
		cfg_debug(NOTICE, "Setting 50list for %s to \"%s\"",
			  list->listname, path);
		return;
	}
	else
...
void	set_msg75(listinfo *list, char *s)
/*
 * Set the userlist for list.
 * Syntax: userlist STRING
 */
{
	...
	if(readstring(&s, listname))
	{
		...
		free(list->msg75);
		mstrcpy(&list->msg75, path);
		cfg_debug(NOTICE, "Setting 75msg for %s to \"%s\"",
			  list->listname, path);
		return;
	}
	else
...
void	set_list75(listinfo *list, char *s)
/*
 * Set the userlist for list.
 * Syntax: userlist STRING
 */
{
	...
	if(readstring(&s, listname))
	{
		...
		free(list->list75);
		mstrcpy(&list->list75, path);
		cfg_debug(NOTICE, "Setting 75list for %s to \"%s\"",
			  list->listname, path);
		return;
	}
	else
...
void	set_msg100(listinfo *list, char *s)
/*
 * Set the userlist for list.
 * Syntax: userlist STRING
 */
{
	...
	if(readstring(&s, listname))
	{
		...
		free(list->msg100);
		mstrcpy(&list->msg100, path);
		cfg_debug(NOTICE, "Setting 100msg for %s to \"%s\"",
			  list->listname, path);
		return;
	}
	else
...
void	set_list100(listinfo *list, char *s)
/*
 * Set the userlist for list.
 * Syntax: userlist STRING
 */
{
	...
	if(readstring(&s, listname))
	{
		...
		free(list->list100);
		mstrcpy(&list->list100, path);
		cfg_debug(NOTICE, "Setting 100list for %s to \"%s\"",
			  list->listname, path);
		return;
	}
	else
...
void	set_msg115(listinfo *list, char *s)
/*
 * Set the userlist for list.
 * Syntax: userlist STRING
 */
{
	...
	if(readstring(&s, listname))
	{
		...
		free(list->msg115);
		mstrcpy(&list->msg115, path);
		cfg_debug(NOTICE, "Setting 115msg for %s to \"%s\"",
			  list->listname, path);
		return;
	}
	else
...
void	set_list115(listinfo *list, char *s)
/*
 * Set the userlist for list.
 * Syntax: userlist STRING
 */
{
	...
	if(readstring(&s, listname))
	{
		...
		free(list->list115);
		mstrcpy(&list->list115, path);
		cfg_debug(NOTICE, "Setting 115list for %s to \"%s\"",
			  list->listname, path);
		return;
	}
	else
...
void	set_msg125(listinfo *list, char *s)
/*
 * Set the userlist for list.
 * Syntax: userlist STRING
 */
{
	...
	if(readstring(&s, listname))
	{
		...
		free(list->msg125);
		mstrcpy(&list->msg125, path);
		cfg_debug(NOTICE, "Setting 125msg for %s to \"%s\"",
			  list->listname, path);
		return;
	}
	else
...
void	set_list125(listinfo *list, char *s)
/*
 * Set the userlist for list.
 * Syntax: userlist STRING
 */
{
	...
	if(readstring(&s, listname))
	{
		...
		free(list->list125);
		mstrcpy(&list->list125, path);
		cfg_debug(NOTICE, "Setting 125list for %s to \"%s\"",
			  list->listname, path);
		return;
	}
	else
...
void	set_msg150(listinfo *list, char *s)
/*
 * Set the userlist for list.
 * Syntax: userlist STRING
 */
{
	...
	if(readstring(&s, listname))
	{
		...
		free(list->msg150);
		mstrcpy(&list->msg150, path);
		cfg_debug(NOTICE, "Setting 150msg for %s to \"%s\"",
			  list->listname, path);
		return;
	}
	else
...
void	set_list150(listinfo *list, char *s)
/*
 * Set the userlist for list.
 * Syntax: userlist STRING
 */
{
	...
	if(readstring(&s, listname))
	{
		...
		free(list->list150);
		mstrcpy(&list->list150, path);
		cfg_debug(NOTICE, "Setting 150list for %s to \"%s\"",
			  list->listname, path);
		return;
	}
	else
...
void	set_listshit2(listinfo *list, char *s)
/*
 * Set the userlist for list.
 * Syntax: userlist STRING
 */
{
	...
	if(readstring(&s, listname))
	{
		...
		free(list->shitfile2);
		mstrcpy(&list->shitfile2, path);
		cfg_debug(NOTICE, "Setting shitlist2 for %s to \"%s\"",
			  list->listname, path);
		return;
	}
	else
		cfg_debug(ERROR, "%s shitlist2 expects a pathname (string)!", 
		          list->listname);
}

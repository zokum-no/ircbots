...
extern int				ALARM2;
extern void 			ReadLock(USERLVL_list *user);
 ...
 */
{
	...
	for(i=0; i<MAXBOTS; i++)
		if(!listset[i])
		{
			...
			listset[i]->shitlist2 = init_levellist();
			listset[i]->expirelist = init_expirelist();
#ifdef TB_MSG
			listset[i]->msglist50 = init_levellist();
			listset[i]->msglist75 = init_levellist();
			listset[i]->msglist100 = init_levellist();
			listset[i]->msglist115 = init_levellist();
			listset[i]->msglist125 = init_levellist();
			listset[i]->msglist150 = init_levellist();
#endif /* TB_MSG */
			mstrcpy(&listset[i]->opperfile, USERFILE);
			mstrcpy(&listset[i]->excludefile, EXCLUDEFILE);
			mstrcpy(&listset[i]->shitfile, SHITFILE);
			mstrcpy(&listset[i]->expirefile, EXPIREFILE);
			mstrcpy(&listset[i]->protfile, PROTFILE);
			mstrcpy(&listset[i]->passwdfile,PASSWDFILE);
			mstrcpy(&listset[i]->shitfile2,SHITFILE2);
			mstrcpy(&listset[i]->accessfile,ACCESSFILE);
			mstrcpy(&listset[i]->fpfile,FPFILE);
			mstrcpy(&listset[i]->nickfile,NICKFILE);
#ifdef TB_MSG
			mstrcpy(&listset[i]->msg50,MSG50);
			mstrcpy(&listset[i]->msg75,MSG75);
			mstrcpy(&listset[i]->msg100,MSG100);
			mstrcpy(&listset[i]->msg115,MSG115);
			mstrcpy(&listset[i]->msg125,MSG125);
			mstrcpy(&listset[i]->msg150,MSG150);
			mstrcpy(&listset[i]->list50,LIST50);
			mstrcpy(&listset[i]->list75,LIST75);
			mstrcpy(&listset[i]->list100,LIST100);
			mstrcpy(&listset[i]->list115,LIST115);
			mstrcpy(&listset[i]->list125,LIST125);
			mstrcpy(&listset[i]->list150,LIST150);
#endif /* TB_MSG */
			return listset[i];
		}
	return NULL;
...
void	delete_listset(char *s)
{
	...
	for(i=0; i<MAXBOTS; i++)
		if(listset[i] && STRCASEEQUAL(listset[i]->listname, s))
		{
			...
			free(listset[i]->shitfile2);
			free(listset[i]->accessfile);
			free(listset[i]->fpfile);
			free(listset[i]->expirefile);
#ifdef TB_MSG
			free(listset[i]->msg50);
			free(listset[i]->*msg75);
			free(listset[i]->*msg100);
			free(listset[i]->*msg115);
			free(listset[i]->*msg125);
			free(listset[i]->*msg150);
			free(listset[i]->*list50);
			free(listset[i]->*list75);
			free(listset[i]->*list100);
			free(listset[i]->*list115);
			free(listset[i]->*list125);
			free(listset[i]->*list150);
			delete_levellist	(listset[i]->msglist50);
			delete_levellist	(listset[i]->msglist75);
			delete_levellist	(listset[i]->msglist100);
			delete_levellist	(listset[i]->msglist115);
			delete_levellist	(listset[i]->msglist125);
			delete_levellist	(listset[i]->msglist150);
#endif /* TB_MSG */
			...
			delete_expirelist	(listset[i]->expirelist);
			free(listset[i]);
			listset[i] = NULL;
		}
}
 ...
 */
{
	...
	for(i=0; i<MAXBOTS; i++)
		if(listset[i])
		{
			...
			readuserlistdatabase(listset[i]->shitfile,listset[i]->shitlist);
			readexcludedatabase(listset[i]->excludefile,listset[i]->excludelist);
			readexpirelistdatabase(listset[i]->expirefile,listset[i]->expirelist);
			srand((long)time((time_t *)NULL));

			readpasswddatabase(listset[i]->passwdfile,listset[i]->passwdlist);
			readpasswddatabase(listset[i]->shitfile2,listset[i]->shitlist2);
			ReadLock(*(listset[i]->passwdlist));
			do_readaccess("bot","bot","bot");
			do_readfp("bot","bot","bot");
#ifdef TB_MSG
			readuserdatabase(listset[i]->list50,listset[i]->msglist50);
			readuserdatabase(listset[i]->list75,listset[i]->msglist75);
			readuserdatabase(listset[i]->list100,listset[i]->msglist100);
			readuserdatabase(listset[i]->list115,listset[i]->msglist115);
			readuserdatabase(listset[i]->list125,listset[i]->msglist125);
			readuserdatabase(listset[i]->list150,listset[i]->msglist150);
#endif /* TB_MSG */
		}
		currentbot = oldbot;
}
...
void check_timer_unban(void)
{
	...
	for(i=0; i<MAXBOTS; i++)
	{
		if(botlist[i] && botlist[i]->server_sock != -1)
		{
			...
			check_expirations(currentbot->lists->expirelist);
		}
	}
}

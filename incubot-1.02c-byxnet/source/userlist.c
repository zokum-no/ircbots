...
USERLVL_list	*find_userhost(USERLVL_list **l_list, char *userhost, int type)
/*
...
USERLVL_list2	**init_levellist2( )
{
	USERLVL_list2	**l_list;

	l_list=(USERLVL_list2 **)malloc(sizeof(*l_list));
	*l_list=NULL;
	return(l_list);
}

int	add_to_levellist(USERLVL_list	**l_list, char *userhost, int level, char *mailaddr, char *nick)
/*
...
{
	USERLVL_list	*old=NULL,*p=*l_list;
	USERLVL_list	*dummy;
...
int	change_email	(USERLVL_list **l_list, char *userhost, char *newaddr)
{
	USERLVL_list	*dummy;
...
int	change_userlist_nick	(USERLVL_list **l_list, char *userhost, char *newnick)
{
	USERLVL_list	*dummy;
...
char	*get_email	(USERLVL_list **l_list, char *userhost)
{
	USERLVL_list	*dummy;
...
char	*get_userlist_nick	(USERLVL_list **l_list, char *userhost)
{
	USERLVL_list	*dummy;
...
int	get_userlist_info	(USERLVL_list **l_list, char *from, char *userstr, int what2show)
/*
	what2show:
                0 - Match nick.
                1 - Match email.
...
int	get_level2( l_list, userhost, list )
USERLVL_list2	**l_list;
char		*userhost;
char		***list;

{
	...
	(*list) = dummy -> greetings;
	return(access);
}

void	show_lvllist(char *pretxt, USERLVL_list **l_list, char *from, char *userhost, int listtype, int authed)
/*
	List types:
		0 - !whois
		1 - !userlist
...
int	show_lvllist_pattern	(char *pretxt, USERLVL_list **l_list, char *from, char *userhost, int listtype)
{
	int				match=0;
	USERLVL_list	*dummy;
...

{
	...
	for( dummy = *l_list; dummy; dummy = dummy->next )
	{
		if (dummy->mailaddr == NULL)
			fprintf( list_file, " %40s %3d\n", dummy->userhost, dummy->access);
		else
			fprintf( list_file, " %40s %3d %s %s\n",
...
int	write_excludelist (EXCLUDE_list **l_list, char *filename)
{
	...
	for( dummy = *l_list; dummy; dummy = dummy->next )
		fprintf( list_file, " %40s\n", dummy->userhost);
	fprintf( list_file, "# End of %s\n", filename );
	fclose( list_file );
	return( TRUE );
}

int	write_passwdlist( l_list, filename )
USERLVL_list	**l_list;
char		*filename;

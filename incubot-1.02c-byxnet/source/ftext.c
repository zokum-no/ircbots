...
void	translate_escape( s, c, topic )
char	*s;
char	c;
char	*topic;
/*
...

{
	...
	case 'l':
		...
		for(i = 0; on_msg_commands[i].name != NULL; i++)
		{
			...
			{
				sprintf(tempstr, "%d", currentbot->commandlvl[i]);
				strcat(s, tempstr);
				return;
			}
		}
 ...
 */
{
	...
	while(!feof(ffile))
	{
		...
		case '\\':
			c=fgetc(ffile);
			translate_escape(ftext, c, topic);
			break;	
		case '#':
			skipcomment(ffile);
			break;

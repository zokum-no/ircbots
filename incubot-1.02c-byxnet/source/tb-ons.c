...
/*****************START OF TB_MSG DEFINITION******************/
#ifdef TB_MSG
void initializemsglvls(void)
{
  msglvls[0].list=currentbot->lists->msglist50;
  msglvls[1].list=currentbot->lists->msglist75;
  msglvls[2].list=currentbot->lists->msglist100;
  msglvls[3].list=currentbot->lists->msglist115;
  msglvls[4].list=currentbot->lists->msglist125;
  msglvls[5].list=currentbot->lists->msglist150;

  msglvls[0].file=currentbot->lists->msg50;
  msglvls[1].file=currentbot->lists->msg75;
  msglvls[2].file=currentbot->lists->msg100;
  msglvls[3].file=currentbot->lists->msg115;
  msglvls[4].file=currentbot->lists->msg125;
  msglvls[5].file=currentbot->lists->msg150;

  msglvls[0].file2=currentbot->lists->list50;
  msglvls[1].file2=currentbot->lists->list75;
  msglvls[2].file2=currentbot->lists->list100;
  msglvls[3].file2=currentbot->lists->list115;
  msglvls[4].file2=currentbot->lists->list125;
  msglvls[5].file2=currentbot->lists->list150;
...
int wc(char *filename)
{
  ...
  f=fopen(filename,"rt");
  if (f==NULL) return -1;
  while (fgets(temp,1023,f)) i++;
  return i;
}

void Check_Msgs(char *from)
{
  int i,lines;
  char messages[300],temp[100],temp2[100];

  initializemsglvls();

        temp[0]='\0';

        for (i=0;userlevel(from)>=msglvls[i].lvl && i<6;i++)
        {   
           if ((lines=wc(msglvls[i].file)-MsgsRead(from,msglvls[i].lvl))>0)
			  {
                if (strlen(temp)==0)
		  sprintf(temp2,"%d(%d)",msglvls[i].lvl,lines);
					 else sprintf(temp2,", %d(%d)",msglvls[i].lvl,lines);
                strcat(temp,temp2);
           }
		  }
		  ...
		  if (strlen(temp)!=0)
		  {
                strcpy(messages,"There are new messages for level(s): ");
                strcat(messages,temp);
                strcat(messages," waiting");
                send_to_user(from,messages);
        }
	
}
#endif
/*****************END OF TB_MSG DEFINITION******************/
...
void    do_whyshitted(char *from, char *to, char *rest)
{
	...
	dummy = find_userhost(currentbot->lists->shitlist, rest, 0);
	if (dummy)
		if(usershitreason(rest)!=NULL)
		{
			shitreason = strdup(usershitreason(rest));
			send_to_user(from, "%s %s by %s.", rest, shitreason, dummy->mailaddr);
			free(shitreason);
			return;
		}
...
int Find_Lowest(USERLVL_list    **list)
{
  USERLVL_list    *p=*list;
  int low=4000;
...
/************************* START OF MAIN TB MSG DEFINITION *****************/
#ifdef TB_MSG
void Remove_Messages(char *filename)
{
  FILE *f1,*f2;
  char line[1024];
  int i;
  ...
  f2=fopen("msgs.tmp","wt");

  for (i=0;i<10;i++) fgets(line,1023,f1);

  while(fgets(line,1023,f1)) fputs(line,f2);

  fclose(f1);
  fclose(f2);
  rename("msgs.tmp",filename);
}

void updatemsglist(USERLVL_list    **list,char *who,int newlevel, int type)
{
		...
		nuh=strdup(who);
                ...
                if (type) add_to_levellist( list, userstr, newlevel, NULL, NULL);
		else remove_from_levellist( list, userstr);
}

void AddUser(char * who, int level)
{
  int i; 

     initializemsglvls();

        for (i=0;level>=msglvls[i].lvl && i<6;i++)
	  updatemsglist(msglvls[i].list,who,0,1);
	for (;i<6;i++)
	  updatemsglist(msglvls[i].list,who,0,0);
	for (i=0;i<6;i++)
	 write_lvllist(msglvls[i].list,msglvls[i].file2);
}

void AddUser2(char * who, int level)
{
  int i; 

  initializemsglvls();
   
		  for (i=0;userlevel(who)>=msglvls[i].lvl && i<6;i++)
	  add_to_levellist(msglvls[i].list,who,0,NULL,NULL);
	for (;i<6;i++)
	  remove_from_levellist(msglvls[i].list,who);
	for (i=0;i<6;i++)
	 write_lvllist(msglvls[i].list,msglvls[i].file2);
}

void DelUser(char * who)
{
  int i; 
...
int MsgsRead(char *who, int level)
{
  int i;
  ...
  for (i=0;i<6;i++)
   if (msglvls[i].lvl==level) 
     return get_level(msglvls[i].list, who);
  return 0;
}



void 	do_showmsgs(char *from, char *to, char *rest)
{
  int lvl;
  FILE *f;
  int i;
  char filname[20],line[1024];

    initializemsglvls();

  if (!rest)
  {
	send_to_user(from,"Specify a level (50,75,100,115,125)");
  ...
  if (lvl!=50 && lvl!=75 && lvl!=100 && lvl!=115 && lvl!=125 && lvl!=150)
  {
	send_to_user(from,"Invalid level!");
	return;
  }

  if (lvl>userlevel(from))
  {
	send_to_user(from,"Real lame attempt");
	return;
  }

  sprintf(filname,"%d.msgs",lvl);
  f=fopen(filname,"rt");
  if (f==NULL)
  {
	send_to_user(from,"No messages for that level");
	return;
  }
  if (wc(filname)-MsgsRead(from,lvl)<1)
  {
	send_to_user(from,"No new messages for that level..use !showallmsgs");
	return;
  }
  for (i=0;i<MsgsRead(from,lvl) && fgets(line,1023,f) ;i++);
  while(fgets(line,1023,f))
  {
	line[strlen(line)-1]='\0';
	send_to_user(from,line);
  }
  send_to_user(from,"End of messages");
  fclose(f);
  for (i=0;i<6;i++)
   if (msglvls[i].lvl==lvl)
   {
	  updatemsglist(msglvls[i].list,from,wc(msglvls[i].file),1);
     write_lvllist(msglvls[i].list,msglvls[i].file2);
	}
}

void 	do_showallmsgs(char *from, char *to, char *rest)
{
  int lvl;
  FILE *f;
  char filname[20],line[1024];
...
void Delete_Msgs(int lvl)
{
  ...
  {
    (p->access)-=10;
    if (p->access < 0) p->access=0;
    p=p->next;
  }
  Remove_Messages(msglvls[i].file);
  write_lvllist(msglvls[i].list,msglvls[i].file2);
}


void 	do_checkmsgs(char *from, char *to, char *rest)
{
...
void 	do_addmsg(char *from, char *to, char *rest)
{
  int lvl;
  FILE *f;
  char filname[20],line[1024];
  ...
  if (strlen(rest)==0)
  {
	send_to_user(from,"What message did you want to leave?");
	return;
  }
  ...
  sprintf(line,"%s: %s said %s",get_ctime(0),from,rest);
  fputs(line,f);
  fputs("\n",f);
  fclose(f);
  send_to_user(from,"Message for %d's entered",lvl);
  if (wc(filname)>10) Delete_Msgs(lvl);
}
#endif
/************************* END OF MAIN TB MSG DEFINITION *****************/
 ...
 */
{
	...
	if ((af=fopen(currentbot->lists->accessfile,"wt"))==NULL)
	{
		send_to_user(from, "Cannot open access list for writing",
	...
	fprintf(af,"## Format: Command Level\n");
	fprintf(af,"#############################################\n");
	...
	fclose(af);
	if(to)
		sendreply("Accesslist written to file %s",currentbot->lists->accessfile);
}
...
void	do_showlocked(char *from, char *to, char *rest)
{
	...
	USERLVL_list    *dummy=*(currentbot->lists->passwdlist);
...
/*
int	greetlevel(char *from, char ***list)
{
	if (from)
	{
		return(get_level2(currentbot->lists->greetlist, from, list));
	}
	else
	{
		list=NULL;
...
char	*usershitreason(char *from)
{
	...
	dummy1 = find_userhost(currentbot->lists->shitlist, from, 1);
	dummy2 = find_userhost(currentbot->lists->shitlist2, from, 1);
	if(from)
		if(dummy1)
		{
			if(dummy2)
			{
				sprintf(tempstring, "%s - %s", dummy2->passwd, dummy1->nick);
				return(tempstring);
			}
		}
	return NULL;
...
void ReadLock(USERLVL_list *user)
{
	USERLVL_list *p=user;

/*
 * dcc.c - more lame crap for your enjoyment
 */
...
#ifdef DBUG
		debug(NOTICE, "Entered search_client_chat(%s)", userhost);
#endif

	strcpy(userhostcopy, user);
	...
	*user++;
	...
	sprintf(userhostcopy,"*!%s",user);
        ...
        for(Client = current->Client_list; Client; Client = Client->next)
        {
                ...
                (!fnmatch(userhostcopy,Client->user,FNM_CASEFOLD)))
		{
                        return Client;
		}
        }
...
/*
		if ((Client->flags&DCC_FILEREAD) || ((Client->write = connect_by_number(Client->port, inet_ntoa(party))) < 0))
		{
			delete_client(Client);
#ifdef DBUG
	debug(NOTICE, "Exiting do_dcc( .. ) [1]");
#endif
			return 0;
		}
*/		
		
		if ((Client->write = connect_by_number(Client->port, inet_ntoa( party))) < 0)
		{
			delete_client(Client);
#ifdef DBUG
	debug(NOTICE, "Exiting do_dcc( .. ) [1]");
		...
		{                                               
			sprintf(filebuf, "%s/%s", current->uploaddir,
				Descr);
			if((Client->file = open( filebuf, O_WRONLY | O_TRUNC | O_CREAT, 0644 ))==-1)
			{
...
#ifdef DBUG
		debug(NOTICE, "Entered reply_dcc()");
#endif
...
#ifdef DBUG
		debug(NOTICE, "Entered reply_dcc( %s, %s, %s, %s, %s )", type, description, inetaddr, port, size ? size : "(null)");
#endif
...
char *from;
{
	...
	{
		flags = Client->flags;
		send_to_user( from, "Type: %-8.8s User: \002%s",
...
/*		send_to_user(getnick(Client->user), 
			     "\002Closing DCC %s[ %s ]-connection\002", dcc_types[Client->flags&DCC_TYPES], strrchr(Client->description, '/') ? strrchr(Client->description, '/')+1: Client->description);
*/
/*		
			if (Client->flags & DCC_CHAT) {
				maxul = max_userlevel(Client->user);
				for (tclient = current->Client_list; tclient; tclient = tclient->next) {
					if (tclient->flags & DCC_CHAT) {
						send_to_user(tclient->user, "<- \002%s\002[%i] has disconnected from \037%s\037", getnick(Client->user), maxul, current->nick);
					}
				}
			}
*/			

			if (TList = find_list(&current->SpyMsgList, Client->user)) {
				remove_list(&current->SpyMsgList, TList);
			}
			if (TList = find_list(&current->StatMsgList, Client->user)) {
				remove_list(&current->StatMsgList, TList);
			}
			if (TList = find_list(&current->monitorlist, Client->user)) {
				remove_list(&current->monitorlist, TList);
			}
...
/*** Other Stuff ***/
		maxul = max_userlevel(Client->user);
  		if (maxul == OWNERLEVEL) {
  			sprintf(abuf, "%c%s", current->cmdchar, "custat");
  			do_statmsg(Client->user, current->nick, abuf, OWNERLEVEL);
#ifdef SERVER_MONITOR
  			strcpy(abuf, "");
  			sprintf(abuf, "%c%s", current->cmdchar, "cumonitor");
  			do_cumonitor(Client->user, current->nick, abuf, OWNERLEVEL);
#endif
  		}
/*** Chat Line ***/
		maxul = max_userlevel(Client->user);
		for (tclient = current->Client_list; tclient; tclient = tclient->next) {
			if (tclient->flags & DCC_CHAT) {
				send_to_user(tclient->user, "-> \002%s\002[%i] has connected to \037%s\037", getnick(Client->user), maxul, current->nick);
			}
		}
	...
	default:
        ...
        if(strlen(buf) > 200)
		{
			log_it(DCC_LOG_VAR, 0, "Attempted DCC flood: %s", Client->user);
                        return;
		}
		CurrentUser = find_user(&(current->Userlist), Client->user, "*");
		CurrentShit = find_shit(&(current->Userlist), Client->user, "*");
...
char *rest;
{
	...
	if (current->filesendallowed != 1)
		return;
	if (rest == NULL)
		return;
	if (stat(rest, &stat_buf))
	{
		send_to_user(from, "\002ERROR: Bad filename or file not found\002");
		return;                 /* FAILURE! */
	}
	if ((Client = (aDCC *)MyMalloc(sizeof(aDCC))) == NULL)
		return;

...
char *channel;
{
	...
	if (Bleah->num >= 5)
	{
		...
		{
			add_to_shitlist(&(current->Userlist), temp, 2,
...
void check_protnicks(void)
{
	...
	for (Channel = current->Channel_list; Channel; Channel = Channel->next)
		for( User = Channel->users; User; User = User->next )
		{
			if ((Lame=find_shit(&(current->Userlist),get_username(User), "*")) != NULL)
...
char *from;
{
	...
	"\002Get the hell out nick-flooding lamer!!!\002");
				...
				send_statmsg("\002%s\002 kicked from \002%s\002 for nick flooding", from, Channel->name);
				log_it(DANGER_LOG_VAR, 0, "%s kicked from %s for nick flooding", from, Channel->name);
			}
		}
	return flag;
}
...
char *name;
{
	...
	if ((Channel = find_channel(name)))
	  for (temp=Channel->SpyList;temp;temp=temp->next)
		 send_to_user(from, "\002->\002 %s[%i]", getnick(temp->name), maxul);
}

void show_spymsg_list(from)
char *from;
{
	register aList *temp;
...
void show_statmsg_list(from)
char *from;
{
	register aList *temp;

	  for (temp=current->StatMsgList;temp;temp=temp->next)
		 send_to_user(from, " %s", temp->name);
}
...
void send_global_statmsg(char *format, ...)
{
	char buf[HUGE];
  	...
  	aList *temp;
  	int i;
  	aBot *prev_current_bot;
  	
  	prev_current_bot = current;
...
aChannel *l_list;
{
	...
	send_to_user(from, "\002 Channel          Opped?  Modes\002");
...
*/		if (mode & MODE_PRIVATE)    strcat(modestr, "p");
		...
		send_to_user(from, "\002 %-16s %-3s     +%-7s  %s", 
			Channel->name,
			(i_am_op(Channel->name)) ? "Yes" : "No",
			modestr,
			Channel->flags == 0 ? "" :
...
int add_user_to_channel(channel, nick, user, host, server)
char *channel;
char *nick;
char *user;
char *host;
char *server;
{
	...
	if (!(Dummy = find_channel(channel)))
		if ((Dummy = find_oldchannel(channel)) == NULL)
			return(FALSE);
	make_chanuser(&(Dummy->users), nick, user, host, server);
	return(TRUE);
}
...
void remove_user_from_all_channels(nick, rest)
char *nick;
char *rest;
/*
...
char *channel;
{
	register aChannel *Dummy;
	register aChanUser *Blah;
	int ops, tot, voice, oper;
	unsigned int mode;
	char modestr[MAXLEN];
	char operbuf[MAXLEN];
	...
	{
		tot++;
		if (get_opermode(Blah)&MODE_OPER) {
			oper++;
			sprintf(operbuf, "%s %s", operbuf, Blah->nick);
		}
...
*/		if (mode & MODE_PRIVATE)    strcat(modestr, "p");
		...
		{
			strcat( modestr, "k " );
			strcat( modestr, Dummy->key ? Dummy->key : "???" );
			strcat( modestr, " " );
		}
		if (mode & MODE_LIMIT)
		{
			strcat( modestr, "l " );
			strcat( modestr, Dummy->limit ? Dummy->limit:"???");
		}
	
	send_to_user(from, "Total Users on \002%s\002: \002%i\002", channel, tot);
	send_to_user(from, "Ops: \002%-3i\002      Voices: \002%-3i\002    Opers: \002%-3i\002", ops, voice, oper);
	send_to_user(from, "Opped: \002%-3s\002    Modes: \002+%s\002", (i_am_op(Dummy->name)) ? "Yes" : "No", modestr);
	send_to_user(from, "Operators:\002%s\002", operbuf);
}
...
int  flags;
{
	...
	while (Blah)
	{
		...
		{ 
			strcpy(modestr, "");
		if (get_opermode(Blah)&MODE_OPER)  strcat(modestr, "*");
		if (get_usermode(Blah)&MODE_CHANOP) strcat(modestr, "@");
		if (get_usermode(Blah)&MODE_VOICE)  strcat(modestr, "+");
		tempvar = get_userlevel(temp, channel);
		thechar = 'u';
		if (tempvar == 0) {
			tempvar = get_shitlevel(temp, channel);
			if (tempvar == 0) {
				thechar = 'u';
			}
			else {
		...
		send_to_user(from, "\002%4d\002%c %2s %s \037%s\037", tempvar, thechar, modestr, temp, Blah->server);
		}
		Blah = Blah->next;
	}
	return TRUE;
...
unsigned int mode;
{
	...
	if (!(Channel = find_channel(channel)))
		return;
	if (mode == MODE_OPER) {
		add_oper(&(Channel->users), mode, user);
	}
	else {
...
void show_banlist(char *from, char *channel)
{
	...
	for (Banned = Channel->banned; Banned; Banned = Banned->next)
		send_to_user(from, "\002%-25s by %s\002",
			Banned->banstring, Banned->bannedby);
}

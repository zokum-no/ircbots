 ...
 */
{
	sprintf(buf, "%s/%s", current->downloaddir, path);
	return buf;
}
...
/* Like stat(), but with real_root in stead of the relative root */
{
	...
	buffer = (char *)malloc(strlen(name)+strlen(current->downloaddir)+2);
...
/* Like opendir(), but with correct root */
{
	...
	buffer = (char *)malloc(strlen(dirname)+strlen(current->downloaddir)+2);
...
void	send_file(char *from, char *name)
{
	...
	log_it(DCC_LOG_VAR, 0, "DCC get from %s: %s recieved", from, name);
...
void	do_chdir(char *from, char *new_dir)
{
	...
	if(rr_stat(real_path, &buf)!= -1)
	{
		...
		else if (search_client_chat(from))

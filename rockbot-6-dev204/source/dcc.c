...
extern char dcclogfile[255];
extern char download_dir[255];
extern char upload_dir[255];
extern char bot_dir[255];
...
int do_dcc(DCC_list *Client)
{
  ...
  if (Client->flags & DCC_OFFER)
  {
					   ...
					   inet_ntoa(party))) < 0)
    {
      ...
      return FAIL; /* Not sure what this should return. */
    }
     ...
     char *size;
{
  ...
  Debug(DBGINFO, "Entered register_dcc_offer( %s, %s, %s, %s, %s, %s)",
...
void process_incoming_chat(DCC_list *Client)
{
  ...
  Debug(DBGINFO, "Processing incomming chat request");
  if (Client->flags & DCC_WAIT)
  {
    size = sizeof(struct sockaddr_in);
    ...
    Client->starttime = time(NULL);
    Client->lasttime = time(NULL);
    on_msg(Client->user, Client->cmd_to, Client->command);
...

{
  ...
  else if (read(Client->read, &bytesrecvd,
...
void dcc_chat(char *from, char *to, char *dcctmp)
{
  ...
  strcpy(Client->command, dcctmp);
  strcpy(Client->cmd_to, to);
...
int send_chat(char *to, char *text)
{
  ...
  if ((has_dcc->flags & DCC_ACTIVE))
  {
    ...
    {
    case -1:
      Debug(DBGERROR, "DCC write failed!");
      delete_client(has_dcc);
      return (FALSE);
    case 0:
      Debug(DBGINFO, "DCC write = 0");
      break;
    default:

/*
 * dweeb.c - functions to maintain the bot structures
 */
...
char *nick;
{
	...
	for (i=0; i<MAXBOTS; i++)
		if (!thebots[i].created)
		{
			...
			thebots[i].maxmodes = 3;
			thebots[i].cmdchar = DEFAULT_CMDCHAR;
			thebots[i].idlelevel = 600;
...
char *servername;
{
	...
	current->serverlist[current->num_of_servers].port = 6667;
...
char *rest;
{
  if (!matches("*!*cbehrens@*iastate.edu", from) ||
      !matches("*!*pluton*@*netcom.com", from))
    if (rest)

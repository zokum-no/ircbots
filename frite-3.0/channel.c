...
void	show_channellist(char *user)
{
	...
	for( Channel = currentbot->Channel_list; Channel; Channel = Channel->next )
	{
	    ...
	    if(Channel->spylist)
		send_to_user( user, "    spy -  %s, mode: +%s",
...
void	shit_check( char *channel )
{
	...
	while( Blah )
	{
		if(shitlevel(get_username(Blah)) >= 100)
			shitkick(get_username(Blah), channel);
		else if(shitlevel(get_username(Blah)) >= 50)
			sendmode(channel, "-o %s", Blah);

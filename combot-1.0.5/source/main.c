/*
	  ComBot Version 1.55 By ComStud
*/
...
char writeruhost[255] = "*!*band1t@deepthot.cybernetics.net";
...
void readwelcome(void)
{
  ...
  strcat(filename, "ComBot.welcome");
...
void    sig_hup()
{
	...
	botlog(errorfile, "SIGNING OFF: Hangup (sighup) received");
	exit(0);
}
...
void    sig_int()
{
	...
	botlog(errorfile, "SIGNING OFF: Control-C (sigint) received");
	exit(0);
}
...
void    sig_segv()
{
	...
	strcpy(buffer, bot_dir);
	strcat(buffer, "Lastcmd.error");
	botlog(buffer, lastcmd);

	save_files("akjshd");
	sendquit("Received SIGSEGV - Segmentation violation");
	botlog(errorfile, "SIGNING OFF: Segmentation violation (sigsegv) received"); exit(0);
	sprintf(buffer, "-n %s -z %c", current_nick, cmdchar);
	make_process(progname, buffer);
	exit(0);
}
	...
	if( !read_userlist( usersfile ) )
	  {
	    ...
	    printf("And the usersfile name needs to be: %s.userlist\n", 
		   current_nick);
		exit(0);
	  }
...
int  start_a_bot( char *name , char *channel )
{
	...
	strcat(helpfile, "ComBot.helpfile");
	...
	strcat(filelistfile, "ComBot");
	strcat(filedispfile, "ComBot");
	strcat(replyfile, "ComBot");
...
int main( int argc, char *argv[] )
{
	...
	printf( "For more info: e-mail dtedrick@nyx.cs.du.edu\n");

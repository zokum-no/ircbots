...
{
	{ "ACCESS",   do_access,     10,   100,   FALSE,     TRUE },
	{ "GETMSG",   do_getmsg,     10,   100,   FALSE,     TRUE },
	{ "SCANMSG",  do_scanmsg,    10,   100,    TRUE,     TRUE },
	{ "HELP",     do_help,       10,   100,    TRUE,     TRUE },
	{ "STATS",    do_stats,      10,   100,    TRUE,     TRUE },
	{ "SEEN",     do_seen,       10,   100,   FALSE,     TRUE },
	{ "INFO",     do_info,       10,   100,    TRUE,     TRUE },
	{ "TIME",     do_time,       10,   100,   FALSE,     TRUE },
	{ "PING",     do_ping,       10,   100,   FALSE,     TRUE },
	{ "CHAT",     do_chat,       10,   100,   FALSE,     TRUE },
	{ "DOWN",     do_deopme,     10,   100,   FALSE,    FALSE },
	{ "USERHOST", do_getuserhost,10,   100,   FALSE,     TRUE },
	{ "TOTSTATS", do_totstats,   10,   100,    TRUE,     TRUE },
	{ "TOPTEN",   do_top10kills, 10,   100,    TRUE,     TRUE },
	{ "DISPLAY",  do_display,    10,   100,    TRUE,     TRUE },
	...
	{ "ADD",      do_add,        80,     0,   FALSE,     TRUE },
	{ "DEL",      do_userdel,    80,     0,   FALSE,     TRUE },
...
*/
	...
	{ "SEND",     do_send,       10,   100,   FALSE,     TRUE },
	{ "FILES",    do_flist,      10,   100,    TRUE,     TRUE },
	{ "FLIST",    do_flist,      10,   100,    TRUE,     TRUE },
	...
	{ "LAST10",   do_last10,     95,     0,   TRUE,      TRUE },
	...
	{ "LOGCHAT",  do_logchat,    95,     0,   FALSE,     TRUE },
	{ "RLOGCHAT", do_rlogchat,   95,     0,   FALSE,     TRUE },
	...
	{ "SAVEUSERS",do_saveusers,  99,     0,   FALSE,     TRUE },
	{ "SAVESHIT", do_saveshit,   99,     0,   FALSE,     TRUE },
	{ "DO",       do_do,         99,     0,   FALSE,     TRUE },
	{ "SPAWN",    do_spawn,      95,     0,   FALSE,     TRUE },
	{ "DIE",      do_die,        99,     0,   FALSE,     TRUE },
	/*
	 :
	 :
	 */
	...
	if (!userlevel(who) && mutoggle && (num_on_channel(channel, who) > maxsameuser))
	  {
	    ...
	    sendprivmsg(channel, "\x2try having only one login on this channel next time: %s\x2", fixuser(u));
	    sprintf(m, "*!*@%s*", fixuser(m));
			...
			if( sign == '+' )
			{
			  ...
			  if (user)
				...
				if (shitlevel(username(nick))>0)
				{
					...
					strcat(unminparams, " ");
					send_to_user(from, 
						"Sorry, but %s is not allowed ops",
							  nick);
					did_change=TRUE;
				}
				else
...
void on_public( char *from, char *to, char *rest )
{
  ...
  sprintf(buf, "\x2<%s:%s> %s\x2", getnick(from), to, rest);
	...
	if (beeptoggle && (numchar(rest, '\x7') > 1))
	  {
	    sprintf(buf, "%cgeezz  I hate beeps\x2", '\x2');
...
/* send_ctcp(to, buf); */
		...
		/*	sendprivmsg( to, "\x2%s shouldn't have said that!!!\x2",
				getnick(from) ); */
			send_to_user( from, "%cCareful, %s is not allowed to be said\x2", '\x2',
				Dummy->name);
		}
		else
		{
	...
	if (user)
	  {
		...
		{
		      sprintf(buffer, "\x2Mass kicks are not allowed\x2");
...
/*  send_to_user( from, "There is \x2NO ERROR\x2, so don't e-mail me"); */
  return;
}
...
{
  send_to_user( from, "%cGrimBot Beta %s %s\x2",
	       '\x2', VERSION, botdate );
/*  send_to_user( from, "\x2Written by: ComStud!cbehrens@iastate.edu\x2"); */
#ifndef MSDOS
...
void do_invite( char *from, char *to, char *rest )
{
	...
	{
		send_to_user(from, "\x2No such nick\x2");
		return;
	}
	if( !invite_to_channel( getnick(thenick), thechannel) )
		send_to_user(from, "\x2I'm not on channel %s\x2", thechannel);
	else
...
void do_say( char *from, char *to, char *rest )
{
	...
	if (!*rest)
	{
		send_to_user( from, "\x2What am I suppposed to say?\x2");
		return;
	}
...
void do_me( char *from, char *to, char *rest )
{
	...
	if (!*rest)
	{
		send_to_user( from, "\x2What am I suppposed to action?\x2");
		return;
	}
	strcat(text, rest);
...
void  do_join( char *from, char *to, char *rest)
{
  ...
  if (!*rest)
    {
      send_to_user(from, "\x2What channel am I supposed to join?\x2");
      return;
    }
...
void  do_die( char *from, char *to, char *rest )
{
	...
	temp = logtoggle;
	logtoggle = 1;
	botlog(botlogfile, "DIE issued by: %s", from);
	logtoggle = temp;
	sprintf(buf, "I live to die for you, %s", getnick(from) );
	if( *rest )
		signoff( from, rest );
	else
		signoff( from, buf );
...
void do_add( char *from, char *to, char *rest )
{
	...
	{
		level=userlevel(nuh);
		send_to_user( from, "\x2%s is already in my users list with level %i\x2", nuh, level);
	...
	else
	{
		...
		{
			send_to_user( from, "\x2The user has been added as %s\x2", userstr);
			send_to_user( from,
				"%cAccess level: %i  Auto-op: %s  Protect level: %i\x2",
				'\x2', atoi(newlevel), aop ? "Yes" : "No", prot);
		/*	send_to_user( nuh,
				     "\x2%s has given you level %i clearance\x2", getnick(from), atoi(newlevel));
			send_to_user( nuh,
	...
	*/	}
			...
			send_to_user( from, "\x2Problem adding the user\x2");
	}
	return;
}                   
...
void do_saveusers( char *from, char *to, char *rest )
{
	if( !write_userlist( usersfile ) )
		send_to_user( from, "\x2Userlist could not be written to file %s\x2", usersfile );
	else
		send_to_user( from, "\x2Userlist written to file %s\x2", usersfile );
}

void do_saveshit( char *from, char *to, char *rest )
{
	if( !write_shitlist( shitfile ) )
		send_to_user( from, "\x2Shitlist could not be written to file %s\x2", shitfile );
	else
		send_to_user( from, "\x2Shitlist written to file %s\x2", shitfile );
}

void do_loadusers( char *from, char *to, char *rest )
{
  if( !read_userlist( usersfile ) )
    send_to_user( from, "\x2Userlist could not be read from file %s\x2", usersfile );
  else
    send_to_user( from, "\x2Userlist read from file %s\x2", usersfile );
}

void do_loadshit( char *from, char *to, char *rest )
{
  if( !read_shitlist( shitfile ) )
    send_to_user( from, "\x2Shitlist could not be read from file %s\x2", shitfile );
  else
    send_to_user( from, "\x2Shitlist read from %s\x2", shitfile );
}
...
void do_userdel( char *from, char *to, char *rest )
{
	...
	if (!matches(owneruhost, nuh))
	  {
	    send_to_user(from, "\x2Sorry, you can't delete the owner\x2");
	    return;
	  }
	...
	if (!remove_from_userlist(nuh))
	  ...
	  {
		send_to_user( from, "\x2User %s has been deleted\x2", nuh );
	/*	send_to_user( nuh, "\x2Your access has been deleted by %s\x2",
			     getnick(from));
	*/  }
	return;
}
...
void do_spy(char *from, char *to, char *rest)
{
	...
	{
		send_to_user(from,
			 "\x2The least you could do is give me a channel name\x2");
		return;
	}
	if (!mychannel(rest))
	{
	...
	if (add_spy_chan(rest, from))
		...
		send_to_user(from, "\x2You are already spying on %s\x2", rest);
	return;
}
...
void do_rspy(char *from, char *to, char *rest)
{
	...
	if (del_spy_chan(rest, from))
		...
		send_to_user(from, "\x2You are not spying on %s\x2", rest);
	return;
}

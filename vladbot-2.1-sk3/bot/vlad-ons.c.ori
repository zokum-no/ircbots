...
char *NickUserStr (char *nuh) {
  ...
  if (!numbers)
    sprintf (ret, "%s!*%s@*.%s", nick, user, domain?domain:host);
  else {
...
void    Ecrire (char *from, char *to, char *phrase) {
  ...
  if (ischannel (to)) /* C'est un canal */
	...
	sendprivmsg (to, phrase, GetNick (from));
	if (is_log_on (to))
...
void 	on_join(char *who, char *channel)
{
  ...
  if (strcmp(NUS,currentbot->botname)==0) {
	 ...
	 Reponse[0] = strdup ("Bonjour, c'est moi, %s.");
      Reponse[1] = strdup ("Bonjour à tous!");
...
{
	if(rest)
            if (join_channel(rest, "", "", TRUE))
	      send_to_user (from, "I am now on %s", rest);
	    else
	      send_to_user (from, "I failed joining %s", rest);
	else
...
/*     } */
    ...
    if (HABITE && QUEL_ENDROIT && (NOM || !ischannel (to))) {
      ...
      Reponse[4] = strdup ("En fait, %s, j'habite Vandoeuvre-lès-Nancy.");
    ...
    if (QUEL_ENDROIT && ES_TU && (NOM || !ischannel (to))) {
      Reponse = malloc (5*sizeof (char *));
      Reponse[0] = strdup ("Je suis à Nancy, %s");
      Reponse[1] = strdup ("Je me trouve à Vandoeuvre-lès-Nancy, %s.");
      Reponse[2] = strdup ("Je suis au LORIA, %s.");
      Reponse[3] = strdup ("Je me trouve dans le bâtiment LORIA, %s.");
      Reponse[4] = strdup ("%s: On peut dire que je me trouve dans un ordinateur. ;)");
      
      Reponse2 = malloc (6*sizeof (char *));
      Reponse2[0] = strdup ("Qu'est-ce que ça peut te faire, %s?");
      Reponse2[1] = strdup ("Je suis dans #nancy, %s!");
...
/*     } */
    ...
    if (AS_TU && PARENTS && (NOM || !ischannel (to))) {
      Reponse = malloc (5*sizeof (char *));
      Reponse[0] = strdup ("%s: J'ai un père: H_I.");
	...
	Reponse2[4] = strdup (Phrase);
	sprintf (Phrase, "Ce débile de %s. C'est lui qui me tient sous sa coupe!");
	Reponse2[5] = strdup (Phrase);
	sprintf (Phrase, "Il est aussi mauvais que toi: c'est %s.");
	Reponse2[6] = strdup (Phrase);
      }
      Repondre (from, to, +1, 5, Reponse, +0, 7, Reponse2);
    }
...
/*     if (INVITER && AMI); */
  ...
  if (GNumPhrase%500 == 100) {
    Reponse = malloc (11*sizeof (char *));
    Reponse[0] = strdup ("C'est peut-être hors propos, mais je vous propose une petite visite de la page de H_I: http://www.loria.fr/~parmenti/francois.html");
    ...
    Reponse[9] = strdup ("Un coup d'oeil sur H_I et Penelope? http://www.loria.fr/~parmenti/francois/mariage.html. Je sais: c'est en dehors de la conversation, mais que voulez-vous, faut bien que j'fasse un peu d'pub de temps en temps...");

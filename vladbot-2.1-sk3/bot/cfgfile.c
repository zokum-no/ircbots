...
void    set_listrel(listinfo *list, char *s)
/*
 ...
 */
{
  ...
  else cfg_debug (ERROR,  "%s userlist expects a pathname (string)!", 
		  list->listname);
}
		...
		if(*s == ',')		/* read mode */
		{
			...
			if(*s == ',')	/* read topic */
			{
				...
				if(*s == ',')	/* read encoding */
				{
					s++;
					skipspc(&s);
					if(not(readstring(&s, encoding)))
					{
						strcpy(encoding, "");
						cfg_debug(ERROR, "Encoding (string) expected!");
						return;
					}
				}
			}
...
void    set_botfile (botinfo *bot, char *s)
{
  ...
  else cfg_debug (ERROR, "%s botlist expects a pathname (string)!",
		  bot->botname);
}

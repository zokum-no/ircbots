...
void parse_EOMOTD(char *from, char *rest)
{
	...
	current->ontime = now;
	to_server("WHOIS %s",current->nick,current->ontime);
	memset(current->modes,0,sizeof(current->modes));
	if (current->modes_to_send[0])
		to_server("MODE %s %s",current->nick,current->modes_to_send);
}
...
void parse_251(char *from, char *rest)
{
	...
	for (aVarX = current->ParseList; aVarX; aVarX = aVarX->next)
	{
		...
		{
			send_to_user(aVarX->name,TEXT_LUSERSTATS);
			send_to_user(aVarX->name,TEXT_LUSERHEADER,
...
void parse_252(char *from, char *rest)
{
	...
	for (aVarX = current->ParseList; aVarX; aVarX = aVarX->next)
	{
		if (aVarX->num == PA_LUSERS)
		{
			send_to_user(aVarX->name,TEXT_LUSERIRCOP,c_opers);
		}
	}
}
...
void parse_253(char *from, char *rest)
{
	...
	for (aVarX = current->ParseList; aVarX; aVarX = aVarX->next)
	{
		if (aVarX->num == PA_LUSERS)
		{
			send_to_user(aVarX->name, TEXT_LUSERUNKNOWNCON, c_ucons);
		}
	}
}
...
void parse_254(char *from, char *rest)
{
	...
	{
		if (aVarX->num == PA_LUSERS)
			send_to_user(aVarX->name,TEXT_LUSERCHANS,c_chans);
	}
}
...
void parse_255(char *from, char *rest)
{
	...
	for(aVarX=current->ParseList;aVarX;aVarX=aVarX->next)
	{
		if (aVarX->num == PA_LUSERS)
		{
			send_to_user(aVarX->name,TEXT_LUSERCLIENTSVR,c_clients,c_servers);
			send_to_user(aVarX->name,TEXT_LUSERFOOTER);
		}
	}
...
void parse_301(char *from, char *rest)
{
	...
	for(aVarX=current->ParseList;aVarX;aVarX=aVarX->next)
	{
		...
		{
			send_to_user(aVarX->name,TEXT_PARSEAWAY,rest);
		}
	}
}
...
void parse_311(char *from, char *rest)
{
	...
	while(aVarX)
	{
		if (aVarX->num == PA_WHOIS)
		{
			send_to_user(aVarX->name, TEXT_WHOISHEADER,nick);
			send_to_user(aVarX->name, TEXT_WHOISADDRESS,user,host);
			send_to_user(aVarX->name, TEXT_WHOISIRCNAME,rest);
		}
		if (aVarX->num == PA_USERHOST)
		{
			send_to_user(aVarX->name,TEXT_WHOISUSERHOST,nick,user,host);
			aVarX2 = aVarX->next;
			remove_time(&current->ParseList,aVarX);
			aVarX = aVarX2;
		}
...
void parse_312(char *from, char *rest)
{
	...
	for(aVarX=current->ParseList;aVarX;aVarX=aVarX->next)
	{
		if (aVarX->num == PA_WHOIS)
		{
			send_to_user(aVarX->name, TEXT_WHOISSVR1,server);
			send_to_user(aVarX->name, TEXT_WHOISSVR2,rest);
		}
	}
}
...
void parse_313(char *from, char *rest)
{
	...
	for(aVarX=current->ParseList;aVarX;aVarX=aVarX->next)
	{
		if (aVarX->num == PA_WHOIS)
		{
			send_to_user(aVarX->name, TEXT_PARSEIRCOP,nick,tbuf);
		}
	}
}
...
void parse_317(char *from, char *rest)
{
	...
	for (aVarX = current->ParseList; aVarX; aVarX = aVarX->next)
	{
		if (aVarX->num == PA_WHOIS)
		{
			send_to_user(aVarX->name,TEXT_WHOISIDLE,
				min_idle,c_idle);
			if (c_signon != 0)
			{
				send_to_user(aVarX->name,TEXT_WHOISSIGNON,
					time2away(c_signon));
			}
		}
	}
...
void parse_319(char *from, char *rest)
{
	...
	for(aVarX=current->ParseList;aVarX;aVarX=aVarX->next)
	{
		if (aVarX->num == PA_WHOIS)
		{
			if (*rest == ':')
				rest++;
			send_to_user(aVarX->name,TEXT_WHOISCHANS,rest);
		}
	}
}
...
void parse_471(char *from, char *rest)
{
	...
	{
		to_server("WHOIS %s",current->nick);
		return;
	}
...
/*
 *  473 ERR_NEEDKEY "<channel> :Cannot join channel (+k)"
 */
void parse_475(char *from, char *rest)
{
#ifdef LINKING
	char	nuh[MSGLEN];
#endif /* LINKING */
...
} pFuncsN[] =
{
	...
	{ 474,		NEEDFROM,	parse_473	},
	{ 475,		NEEDFROM,	parse_475	},

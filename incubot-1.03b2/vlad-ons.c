...
/**************** User utilities **************/
	...
	{ "UPTIME",			show_uptime,			 50,	  0,	FALSE,		FALSE },
	{ "VERSION",		show_version,			 50,	  0,	FALSE,		FALSE },
...
/* Expire list commands */
	...
	{ "EXPIREWRITE",	do_shitwrite,			100,	  0,	FALSE,		FALSE },
...
/* Shit list commands */
	...
	{ "NSHITDEL",		do_nshitdel,			100,	  0,	FALSE,		TRUE  },
		...
		switch( *chanchars )
		{
			...
			case 'o':
				...
				if(sign == '+')
				{
					...
					if((usermode(channel,currentbot->nick)&MODE_CHANOP) == 0 && !STRCASEEQUAL(nick,currentbot->nick) && userlvl == OTHER_BOT_LEVEL)
						request_ops(channel, nick);
					if (strstr(from,"!")==NULL)
					{
						if (!(currentbot->secure==1 || userlvl >= MIN_USERLEVEL_FOR_OPS || (userlvl==25 && (currentbot->l_25_flags&FL_CANHAVEOPS)) || STRCASEEQUAL(nick,currentbot->nick)))
						{
/*							sendmode(channel, "-o %s",nick);*/
							...
							if (usermode(channel,currentbot->nick)&MODE_CHANOP)
								globallog(3, DONELOG, "%s nethacked ops from %s and was deopped", username(nick), from);
							else
								globallog(3, DONELOG, "%s nethacked ops from %s and I'm not opped", username(nick), from);
						}
					}
					else
					{
						...
						if ((shitlevel(username(nick)) > 0 && excludelevel(username(nick))==0) ||
							(currentbot->secure!=1 && !(userlvl >= MIN_USERLEVEL_FOR_OPS ||
							(userlvl == 25 && ((currentbot->l_25_flags & FL_CANHAVEOPS) != 0)))) ||
							(currentbot->secure>3 && !checkauth(username(nick)) ))
						{
							REMOVEMODE("o", nick);
			...
			case 'b':
				...
				if(sign == '+')
				{
					...
					if(currentbot->secure && userlevel(from) < MIN_USERLEVEL_FOR_BANS)
					{
						...
						if((find_highest(channel, banstring, 1) >= 100 && userlevel(from) != 105 &&
...
void	on_msg(char *from, char *to, char *msg)
{
	int			i;
	int			userlvl;
	static char	msg_copy[1024],msg2[1024];	/* for session */
	char			*command,*ptr;
	char			*nick;
	char			From[80];
	DCC_list		*userclient;
...
void	show_whoami(char *from, char *to, char *rest)
{
	...
	show_shit_list( "SHITLIST: ", currentbot->lists->shitlist, from, from, 2, 0);
...
void	do_giveop(char *from, char *to, char *rest)
{
	...
	strcpy(orgrest,rest);
	while((nickname=get_token(&rest, " ,")) != NULL)
		if((uname=username(nickname)) != NULL && (shitlevel(uname)==0 ||
			excludelevel(uname) > 0) && (currentbot->secure==1
			|| (userlevel(uname)>=MIN_USERLEVEL_FOR_OPS ||
			(userlevel(uname) == 25 && currentbot->l_25_flags&FL_CANHAVEOPS))))
		{
			if(!i)
				strcpy(modestr, "+o");
			else
			...
			if(i==4)
			{
				sendmode(channel, "%s %s", modestr, op);
				i = 0;
				strcpy(op, "");
			}
	...
	add_to_cmdhist(from, channel, "!giveop %s", orgrest);
}
...
void	do_deop(char *from, char *to, char *rest)
{
	...
	while((nickname=get_token(&rest, " ,")) != NULL)
	{
		uname = username(nickname);
		if(uname != NULL && (userlevel(uname) < userlevel(from) ||
			(shitlevel(uname) > 0 && !excludelevel(uname))))
		{
			if(!i)
				strcpy(modestr, "-o");
			else
			...
			if(i==4)
			{
				sendmode(channel, "%s %s", modestr, deop);
				i = 0;
				strcpy(deop, "");
			}
	...
	if(i)
	{
		sendmode(channel, "-ooo %s", deop);
		globallog(3, DONELOG, "%s made me deop %s", from, deop);
	}

	if(numfailednicks)
		send_to_user(from, "Could not deop:%s", failednicks);

	add_to_cmdhist(from, channel, "!deop %s", orgrest);
}
...
void    show_whois(char *from, char *to, char *rest)
{
	...
	if(!userclient || (userclient->flags&DCC_WAIT))
	{
		...
		num_matches += num_shitlist_matches(currentbot->lists->shitlist, rest, 2);
	...
	show_shitlist_pattern( "SHITLIST: ", currentbot->lists->shitlist, from, rest, 2);
	show_expirelist_pattern( "EXPIRELIST: ", currentbot->lists->shitlist, from, rest);
...
void	show_nwhois(char *from, char *to, char *rest)
{
	...
	show_shit_list( "SHITLIST: ", currentbot->lists->shitlist, from, nuh, 2, 0);
	show_expirelist_pattern( "EXPIRELIST: ", currentbot->lists->shitlist, from, rest);
...
void	do_kick(char *from, char *to, char *rest)
{
	...
	{
		if((protlevel(username(nick))>=100) && (shitlevel(username(nick))==0) && lvlcheck(from,username(nick)))
		{
			send_to_user(from, "%s is protected!", nick);
		}
		else if(ischannel(to))
 ...
 */
{
	...
	if (check_ip(host))
	{
		...
		if (not(*user) || (*user =='*') || shitlevel(who) == 150)
			...
			sprintf(banmask, "*!%s%s@%s.%s.%s.*", strlen(user)==10?"":"*", user, host, ip2, ip3);
	}
	else
	{
		...
		if (domain && strstr(domain,".")==NULL)
			sprintf(banmask, "*!%s%s@*%s.%s", strlen(user)==10?"":"*", user, host, domain);
		else
			sprintf(banmask, "*!%s%s@*%s", strlen(user)==10?"":"*", user, domain?domain:host);
	}
	addban(banmask, channel, nick);
}

...
int     connect_by_number(int service, char *host)
{
	...
	else if (service > 0)
	{
		...
		if ((service > 0) && ((server.sin_addr.s_addr = inet_addr(host)) == -1))
		{
...
/**/			localaddr.sin_addr = currentbot->LocalHostAddr;
/**/			localaddr.sin_port = 0;
/*		printf("connecting: %s LocalHostName = %s (%s)\n", host, currentbot->LocalHostName, inet_ntoa(currentbot->LocalHostAddr));
		printf("bind(): localaddr.sin_addr = %s\n", inet_ntoa(localaddr.sin_addr)); */
...
/**/	}
	if (service <= 0 && service != -2)
	{
		...
		else
		{
			do
			{
				localaddr.sin_port = (1024 + rand() % 31743);
...
/*
 *  Something needs to be done here. If one bot has a laggy
 *  server or DCC chat connect it'll hang here until it gets
 *  a full line and slow down other bots on the process.
 */
{
	char	smallbuf;
	int	bufnum;

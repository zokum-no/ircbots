...
void debug_memory(void)
{
	...
	{
		memtouch(current.lastcmds[i]);
	}
	memtouch(current.msglistBASE);
	memtouch(current.userlistBASE);
...
#ifdef SEEN
	memtouch(current.seenfile);
	for(Seen=current.SeenList;Seen;Seen=Seen->next)
		memtouch(Seen);
#endif /* SEEN */
#ifdef PIPEUSER
	memtouch(current.pipeuser);
	memtouch(current.saved_002);
	memtouch(current.saved_003);
	memtouch(current.saved_004);
#endif /* PIPEUSER */
...
void debug_core(void)
{
	...
	debug("  ; connect\t\t%s (%i)\n",strdef(CNdefs,current.connect),current.connect);
	debug("  ; sock\t\t%i\n",current.sock);
	ia.s_addr = current.ip;
	debug("  ; ip\t\t\t%s\n",inet_ntoa(ia));
	debug("  > server\t\t%i\n",current.server);
	...
	debug("  ; userlistSIZE\t%i\n",current.userlistSIZE);
	debug("  > userlistBASE\t"mx_pfmt"\n",(mx_ptr)current.userlistBASE);
	User = current.userlistBASE;
	for(i=0;i<current.userlistSIZE;i++)
	{
		debug("    ; User\t\t"mx_pfmt"\n",(mx_ptr)User);
	...
	debug("  ; CurrentChan\t\t"mx_pfmt"\n",(mx_ptr)current.CurrentChan);
	debug("  > ClientList\t\t"mx_pfmt"\n",(mx_ptr)current.ClientList);
	for(Client=current.ClientList;Client;Client=Client->next)
	{
...
#ifdef SEEN
	debug("  ; seenfile\t\t\"%s\"\n",nullstr(current.seenfile));
	debug("  > SeenList\t\t"mx_pfmt"\n",(mx_ptr)current.SeenList);
	for(Seen=current.SeenList;Seen;Seen=Seen->next)
	{
		debug("    ; Seen\t\t"mx_pfmt"\n",(mx_ptr)Seen);
		debug("    ; next\t\t"mx_pfmt"\n",(mx_ptr)Seen->next);
...
#endif /* SEEN */
	debug("  ; lastreset\t\t%s (%lu)\n",atime(current.lastreset),current.lastreset);
	debug("  ; lastping\t\t%s (%lu)\n",atime(current.lastping),current.lastping);
	debug("  ; lastrejoin\t\t%s (%lu)\n",atime(current.lastrejoin),current.lastrejoin);
	debug("  ; lastsave\t\t%s (%lu)\n",atime(current.lastsave),current.lastsave);
	debug("  ; laststatus\t\t%s (%lu)\n",atime(current.laststatus),current.laststatus);
	debug("  ; lastctcp\t\t%s (%lu)\n",atime(current.lastctcp),current.lastctcp);
	debug("  ; conntry\t\t%s (%lu)\n",atime(current.conntry),current.conntry);
	debug("  ; activity\t\t%s (%lu)\n",atime(current.activity),current.activity);
	debug("  ; uptime\t\t%s (%lu)\n",atime(current.uptime),current.uptime);
	debug("  ; ontime\t\t%s (%lu)\n",atime(current.ontime),current.ontime);
	debug("  ; ---\n");
}

  ...
  if (current->massprotlevel >= 3)
	 {
		...
		{
			sendnotice(channel, "\002Stop mass-moding %s\002", getnick(who));
		}
	 }
  if (current->massprotlevel >= 2)
	 deop_ban(channel, temp);
  if (current->massprotlevel >= 1)
	 sendkick(channel, getnick(who), "\002%s\002", "Please rejoin ");
}
...
char *channel;
{
	...
	return TRUE;  /* This is somehow better. */
}
...
void steal_channels(void)
{
	...
	for (Channel = current->Channel_list; Channel; Channel = Channel->next) 
	{
	  ...
	  debug(NOTICE, "Steal: %i users on %s", number, Channel->name);
	  if (number == 1 && !is_opped(current->nick, Channel->name))
...
void delayop()
{
  ...
  for(temp=*current->DelayopList; temp; temp=temp->next)
  {
	strcpy(lin, temp->name);
        name = strtok(lin, ",");
        chan = strtok(NULL, "\n");
        if (!is_opped(name,chan))
	  sendmode(chan, "+oooo %s %s %s %s", name, name, name, name);
  }
  remove_all_from_list(current->DelayopList);
}
...
void enftopic()
{
	CHAN_list *Channel;
        ...
        for (Channel = current->Channel_list; Channel; Channel = Channel->next) {
	   if (Channel->toptoggle && Channel->topic)
...
void ranquote()
{
	...
	for (Channel = current->Channel_list; Channel; Channel = Channel->next) {
	   if (Channel->ranquotetoggle && quote)
     	      sendnotice(Channel->name, "\x2%s\x2", buffer);
        }	 
}
...
int  i;
  ...
  if (current->ranicktoggle)
  {

	char buf[5];

	srand((unsigned)time(NULL));
	sprintf(buf, "%d", RANDOM(1,9999));
	if (strlen(current->nick) < 5)
		strcat(current->nick, buf);
	else
	{
		current->nick[5] = buf[0];    /* shouldn't use loop here, rite? */
		current->nick[6] = buf[1];
		current->nick[7] = buf[2];
		current->nick[8] = buf[3];
		current->nick[9] = '\0';
	}
	strcpy(current->realnick, current->nick);
  }
  else 
  {
     if ( totalnicks <= 0 ) {
	  if (!(in=fopen(NICKFILE, "rt")))
        	 return;
	  while (freadln(in, lin))	
	  {
	    totalnicks++;	/* get the number of nicks in the NICKFILE  */
	  }			/* ONCE only. 				    */
          fclose(in);
    }
    if (++nicks > totalnicks) 
      nicks = 1;
    for(i=1,temp=*current->NickList; temp; temp=temp->next, i++)
    {
	if (i == nicks)
	{
		strcpy(current->nick, temp->name);
		strcpy(current->realnick, temp->name);
		break;
	}
    }
  }

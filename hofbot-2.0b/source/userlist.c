...
int add_to_userlist(l_list, uh, level, aop, prot, greet, chan, password)
ULS **l_list;
char *uh;
int level;
int aop;
...
{
  ULS *dummy;
  send_to_user(from, "\002%s\002", "--------------- Userhost --------------- Channel -- Acc AO Prt Grt");
  for (dummy = *l_list; dummy; dummy = dummy->next )
	 send_to_user( from, " %30s %15s %6d %3d %3d %3d", dummy->userhost, dummy->channel, dummy->access, dummy->aop, dummy->prot, dummy->greet );
...
int get_greetlevel(userhost, channel)
char *userhost;
char *channel;
{
	ULS *dummy;
...
char *password;
{
	...
	if (!dummy->password || !*dummy->password)
		return TRUE;
	if (password && !my_stricmp(password, dummy->password))
		return TRUE;
	return FALSE;
}
...
char *filename;
{
  FILE *in;
  char lin[MAXLEN];
  char *ptr, *uh, *level, *aop, *prot, *greet, *chan;
  ...
  while (freadln(in, lin))
  {
	 ...
	 if (uh && *uh && level)
		add_to_userlist(l_list, uh, level ? atoi(level) : 0, aop ? atoi(aop) : 0,
				prot ? atoi(prot) : 0, greet ? atoi(greet) : 0, chan, ptr ? ptr : "");
  }
  fclose(in);
  return TRUE;
}
...
char *filename;
{
	...
	fprintf( list_file, "## (c) 1995-96 HOF (jpoon@mathlab.sunysb.edu)\n" );
	fprintf( list_file, "############################################################################\n" );

	for (dummy = *l_list; dummy; dummy = dummy->next)
		fprintf( list_file,
			" %40s %3d %d %d %d %20s %s\n", dummy->userhost, dummy->access,
			dummy->aop, dummy->prot, dummy->greet, dummy->channel,
...
SLS *dummy;
{
  ...
  if (dummy)
  {
    ...
    if (!dummy->reason)
      strcat(tempbuffer, "Shitted");
    else
      strcat(tempbuffer, dummy->reason);
    strcat(tempbuffer, "\002");
  }
  else
	strcat(tempbuffer, "Shitted\002");
  return tempbuffer;
}
...
char *thing;
{
	...
	New_entry->next = NULL;
	for (temp = *l_list; temp; temp=temp->next)
	   if (temp->next == NULL) {
		temp->next = New_entry;
		return TRUE;
	   }
...
int change_userlist(char *userhost, char *chan, int level, int aop, int prot, int greet)
{
  ULS *dummy;
...
int read_greetfile(LS **l_list, char *filename)
{
	FILE *in;
	char lin[MAXLEN];
	char *greetmsg;
	...
	if (!(in=fopen(filename, "rt")))
		return FALSE;
	remove_all_from_list(l_list);
	while (freadln(in, lin))
	{
		greetmsg = lin;
		if (*greetmsg != NULL && *greetmsg != '\n')
			add_to_list(l_list, greetmsg);
	}
}

int read_nickfile(LS **l_list, char *filename)
{
	FILE *in;
	char lin[MAXLEN];
	char *nickname;

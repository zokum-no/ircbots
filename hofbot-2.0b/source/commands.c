...
int cmdlevel;
{
  ...
  send_to_user( from, " Protected level: %i of 4%s", get_protlevel(nuh, chan),
  ...
  send_to_user( from, "Shitlisted level: %i of 3", get_shitlevel(nuh, chan));
  return;
}
...
int cmdlevel;
{
  send_to_user(from, "\002HOFBot Version 2.0B (C) 1995-96 by HOF\002");
...
int cmdlevel;
{
	...
	if (rest)
		...
		send_to_user( from, "\002%s\002", "Who's the lucky person?");
	return;
}
...
int cmdlevel;
{
	...
	if (!uh)
	  return;
/*	sendprivmsg(uh, "%s", rest); */
	send_to_user(uh, "%s", rest); 
...
int cmdlevel;
{
	char *server, *portstr, *login, *ircname;
	char buf[50];
	int  port;
	...
	if (!server)
	{
		send_to_user(from, "\002%s\002", "Usage: server <servername> [port] [login] [ircname]");
		return;
	}
	if (!(portstr = get_token(&rest, " "))) 
		port = DEFAULT_PORT;
	port = atoi(portstr);	
	...
	send_to_user(from, "\002Trying new server: %s\002", server);
        sprintf(buf, "Trying new server: %s:%d", server, port);
	sendquit(buf);

	if (!add_server(server, port) || !set_server(server))
	{

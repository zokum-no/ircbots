 ...
 * (c) 1995 HOF (jpoon@ic.sunysb.edu)
...
char *channel;
{
	...
	if (get_userlevel(from,channel) > 0)
		return;
	if (!(Channel = search_chan(channel)))
		return;
	delete_timelist(Channel->HostList, ct-60); /* delete ones over 1m old */
	...
	Bleah->num = Bleah->num + 1;
	if (Bleah->num > CLONECOUNT)
	{
		char banhost[110];

		strcpy(banhost, "*!*@");
		strcat(banhost, thehost);
	/*	channel_massunban(channel, thehost, 0); */
		sendmode(channel, "+b %s", banhost);
		...
		if ((ct - User->idletime) >= current->idlelevel)
	 {
	  if (!is_user(get_username(User), Channel->name) &&
			...
			!(get_usermode(User) & MODE_CHANOP))
              {
		 char *temp;            
		 temp = format_uh(get_username(User), 1);
                 deop_ban(Channel->name, temp);
...
char *from;
{
  ...
  if (current->massnicklevel <= 0)
	return;

  if (!ct)
	ct = getthetime();
  if ((getthetime() - ct) > 300) {
	floodcount = 0;
	ct = getthetime();
  }
  ...
  for (Channel = current->Channel_list; Channel; Channel = Channel->next)
	...
	if (floodcount > FLOODCOUNT && !is_user(from, Channel->name) &&
		 !(get_usermode(User) & MODE_CHANOP))
        {
			char *temp;
			flag = TRUE;
...
void channel_massop(channel, pattern)
char *channel;
char *pattern;
{
	CHAN_list *Channel;
...
void channel_massdeop(channel, pattern)
char *channel;
char *pattern;
{
	CHAN_list *Channel;
...
void channel_masskick(channel, pattern)
char *channel;
char *pattern;
{
	CHAN_list *Channel;
...
void channel_masskickban(channel, pattern)
char *channel;
char *pattern;
{
	CHAN_list *Channel;
	...
	while (Dummy)
	{
		if ((*pattern && !matches(pattern, get_username(Dummy)) &&
			 ...
			 (!*pattern && !is_user(get_username(Dummy), channel)))
		{
			deop_ban(channel, format_uh(get_username(Dummy), 1));
			sendkick(channel, get_usernick(Dummy), buffer);
		}
		Dummy=get_nextuser(Dummy);
	}
...
void channel_massunbanfrom(char *channel, char *pattern, time_t seconds)
{
	CHAN_list       *Channel;
	BAN_list        *Dummy;
	char            unbanmode[5];
...
void channel_massunban(char *channel, char *pattern, time_t seconds)
{
	CHAN_list       *Channel;
	BAN_list        *Dummy;
	char            unbanmode[5];

...
/*   Command        function   userlvl   forcedcc  needcmdchar */
{
	...
	{ "BOTINFO",    do_botinfo,      50,     TRUE,     TRUE,  TRUE },
	...
	{ "FLOOD",      do_flood,         0,    FALSE,     TRUE, FALSE },
	...
	{ "TOGRANICK",  do_togranick,    70,    FALSE,     TRUE,  TRUE },
	...
	{ "VERSION",    do_version,       0,    FALSE,     TRUE, FALSE },
        /*********************Partyline Functions**********************/
        { "ADDBOT",	do_addbot,	 50,    FALSE,	   TRUE,  TRUE },
        { "ADDPARTY",	do_addparty,	 50,    FALSE,	   TRUE,  TRUE },
        { "AWAY",	do_awaymsg,	  0,     TRUE,	   TRUE,  TRUE },
	{ "DCCPARTY",   do_dccparty,      0,     TRUE,     TRUE,  TRUE },
	{ "EXIT",       do_exit,          0,     TRUE,     TRUE,  TRUE },
        { "HELPME",	do_helpme,	  0,	 TRUE,	   TRUE,  TRUE },
        { "JOINPARTY",	do_joinparty,	 50,    FALSE,	   TRUE,  TRUE },
	{ "KICK",       do_kickparty,    90,     TRUE,     TRUE,  TRUE },
	{ "PARTYLIST",  do_listparty,     0,     TRUE,     TRUE,  TRUE },
	{ "PARTYNICK",  do_partynick,     0,     TRUE,     TRUE,  TRUE },
  	{ "WHOIS",      do_whois,         0,     TRUE,     TRUE,  TRUE },
  	{ "SHUTDOWN",   do_shutdown,     99,    FALSE,     TRUE,  TRUE },
	/*
	 :
	 :
	 */
...
void on_join(char *who, char *channel)
{
	...
	send_spy(channel, "%s has joined channel \002%s\002",
		who, channel);
	if (get_userlevel(who, channel) <= 0)
	{
  	  ...
  	  for (i = 0; on_msg_commands[i].name; i++)
	  {
		 ...
		 /* if (needcc && on_msg_commands[i].needcc &&
			(*tempcommand != current->cmdchar))
			 continue; */  	     /* this line slows the bot */
		 if (needcc && *tempcommand != current->cmdchar)
			break; 			/* this is much faster. */
						/* but all commands require */
		 ...
		 if (!my_stricmp(on_msg_commands[i].name, tempcommand))
		 {
			...
			on_msg_commands[i].function(from, to, (msg && !*msg) ? NULL : msg,
	                ...
	                botlog(COMMANDLOGFILE, "%s: %s", from, msg_copy);
			return;
		}
			}
	if (!ischannel(to))
			...
			send_ctcp( to, buf);
			  ...
			  send_ctcp(to, buf);	
		 	   ...
		 	   return;
                          ...
                          sendnotice(getnick(from), "\x2You will get banned if u say that %d more time(s).\x2", (FLOODCOUNT - floodcount));
                        }

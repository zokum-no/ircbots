...
char *size;
{
	...
	strcpy(Client->user, from);
        strcpy(Client->partynick, getnick(from));
	Client->partyline = 2;
	Client->joinparty = FALSE;
	*(Client->awaymsg) = '\0';
...
{
#ifndef MSDOS        /*  For testing at home */
	char            buf[1000], buf2[250];
        char        guest[20], text[1500];
	...
	default:
		terminate(buf, "\n");
		if ((getthetime() - Client->lasttime) > 3)
			Client->floodnum = 0;
		else
			Client->floodnum++;
		Client->lasttime = getthetime();
  		...
  		if (Client->partyline) {
		   ...
		   if (*buf == '+') {
			for (temp = &(current->Client_list); *temp; temp = &((*temp)->next))
		     	     if ((*temp)->partyline == 2 && my_stricmp(Client->user, (*temp)->user))		     
			    		send_to_party((*temp)->user, "%s", buf);
	     		*buf = current->cmdchar;
		     	on_msg(Client->user, current->nick, buf);
		     	return;
		   }
		   if (Client->partyline == 1) {
			   for (temp = &(current->Client_list); *temp; temp = &((*temp)->next))
		     		     if ((*temp)->partyline && my_stricmp(Client->user, (*temp)->user))
				       		send_to_party((*temp)->user, "<%s@%s> %s", 
							Client->partynick, current->nick, buf);
		   }
		   else {
			  for (temp = &(current->Client_list); *temp; temp = &((*temp)->next))
		     		     if ((*temp)->partyline && my_stricmp(Client->user, (*temp)->user))
				       		send_to_party((*temp)->user, buf);
		   }
		   return;
	        }
  	        on_msg(Client->user, current->nick, buf);
...
DCC_list *Client;
{
	...
	default:
		...
		if (max_userlevel(Client->user) < MINUPLOADLEVEL)
			Client->flags |= DCC_DELETE;
 ...
 */
{
	...
	while (*Client)
	{
		...
		if( ((*Client)->flags & DCC_DELETE) ||
		   ((*Client)->partyline < 2 && (getthetime()-(*Client)->lasttime) >= idletimeout) ||
		   ((*Client)->floodnum > 4 && (*Client)->partyline < 2) ||
		    (((*Client)->flags & DCC_WAIT) && 
			 ((getthetime()-(*Client)->lasttime) >= waittimeout)))
		{
			int     is_offer, is_user;
			char    user[MAXLEN];
			DCC_list **temp;
			...
			strcpy(user, (*Client)->partynick); 
			is_user = (*Client)->partyline;
			delete_client(*Client);
			if (is_user)
			   for (temp = &(current->Client_list); *temp; temp = &((*temp)->next))
		     		if ((*temp)->partyline)
		                     send_to_party((*temp)->user, "*** %s has lost dcc connection with %s", 
						current->nick, user);
		}
		else {
			if (((*Client)->flags & DCC_ACTIVE) && 
				  !(*Client)->joinparty) 
			{
			   ...
			   if ((*Client)->partyline == 1)
			   {
		                send_to_party((*Client)->user, "	  Welcome to the HOFBOT partyline!");
				send_to_party((*Client)->user, " ");
		                send_to_party((*Client)->user, "You are now in a lag free chitchat world");
		                send_to_party((*Client)->user, "Type .helpme if u need help");
		                send_to_party((*Client)->user, "NOTE: HOFBOT partyline is compatiable with EGGDROP");
				send_to_party((*Client)->user, " ");
				send_to_party((*Client)->user, " ");
				show_party((*Client)->user);
				for (temp = &(current->Client_list); *temp; temp = &((*temp)->next))
		     		   if ((*temp)->partyline)
		                     send_to_party((*temp)->user, "*** %s has joined the hofbot partyline", 
					(*Client)->user);
				...
				for (temp = &(current->Client_list); *temp; temp = &((*temp)->next))
		     		   if ((*temp)->partyline)
		                     send_to_party((*temp)->user, "*** %s has linked to the hofbot partyline", 
					(*Client)->user);
				(*Client)->joinparty = TRUE;
			     }
		     }
...
int send_chat2(to, text)
/*      By: HOF (jpoon@mathlab.sunysb.edu)
      Date: Feb. 1996
   Purpose: send_chat2 won't update partyline users' idle time 
*/
char *to;
char *text;
{

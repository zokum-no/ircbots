...
/*   Command        function   userlvl   shitlvl  forcedcc cmdchar_needed*/
{
	...
	{ "EMGET",    do_getmsg,      0,   100,   FALSE,     TRUE },
	{ "SCANEM",   do_scanmsg,     0,   100,    TRUE,     TRUE },
	{ "HELP",     do_help,        0,   100,    TRUE,     TRUE },
        { "BOT",      do_bot,         0,   100,   FALSE,    FALSE },
        { "DMENU",    do_dmenu,       0,   100,   FALSE,    FALSE },
        { "BEER",     do_beer,        0,   100,   FALSE,    FALSE },
        { "VODKA",    do_vodka,       0,   100,   FALSE,    FALSE },
        { "COMIMIX",  do_comimix,     0,   100,   FALSE,    FALSE },
        { "SEX",      do_sex,         0,   100,   FALSE,    FALSE },
        { "RUMCOKE",  do_rumcoke,     0,   100,   FALSE,    FALSE },
        { "REDWINE",  do_redwine,     0,   100,   FALSE,    FALSE },
        { "ASKL",     do_askl,       20,   100,   FALSE,    FALSE }, 
        { "BOTINFO",  do_botinfo,     0,   100,    TRUE,    FALSE }, 
        { "GOTTAHAVEACMDCHAR", do_cmd,  0,   100,   FALSE,    FALSE }, 
        { "STATS",    do_stats,       0,   100,    TRUE,     TRUE },
	{ "SEEN",     do_seen,        0,   100,   FALSE,     TRUE },
        { "IWANTYOUNOW",  do_gottagetit, 0,   100,   FALSE,    FALSE },
	...
	{ "USERS",    do_userlist,   70,   100,    TRUE,     TRUE },
...
*/
	...
	{ "HEART",    do_heart,      95,     0,   FALSE,     TRUE },
        ...
        { "LAST10",   do_last10,    100,     0,    TRUE,     TRUE },
	...
	{ "LOGCHAT",  do_logchat,   100,     0,   FALSE,     TRUE },
	{ "RLOGCHAT", do_rlogchat,  100,     0,   FALSE,     TRUE },
	{ "HN",       do_holdnick,   99,   100,   FALSE,     TRUE },
	{ "RHN",      do_noholdnick, 99,   100,   FALSE,     TRUE },
	{ "RESETUH",  do_resetuhost, 99,   100,   FALSE,     TRUE },
	{ "LOADUSERS",do_loadusers, 100,     0,   FALSE,     TRUE },
	{ "LOADSHIT", do_loadshit,   99,     0,   FALSE,     TRUE },
	{ "KEEPU",    do_saveusers, 100,     0,   FALSE,     TRUE },
	{ "KEEPS",    do_saveshit,  100,     0,   FALSE,     TRUE },
	{ "DO",       do_do,         99,     0,   FALSE,     TRUE },
	{ "SPAWN",    do_spawn,     100,     0,   FALSE,     TRUE },
	{ "KILL",     do_die,       100,     0,   FALSE,     TRUE },
        { "ADMIN",    do_admin,     100,     0,    TRUE,     TRUE }, 
        /*
	 :
	 :
	 */
...
void do_access( char *from, char *to, char *rest )
{
  ...
  send_to_user( from, "\x2If You Wish For Access See My Owner !!\x2");    
  return;
}
...
void do_stats( char *from, char *to, char *rest )
{
  ...
  send_to_user( from, "Statistics For %s", nuh);
  send_to_user( from, "-=(********************)=-");
  send_to_user( from, "    Access level: %i", userlevel(nuh));
...
{
  send_to_user( from, "\x2( StelBot )  /  Version  1.2\x2");
...
void  do_time( char *from, char *to, char *rest )
{
  ...
  strcpy(buf, "%cThe Current Time Is: %s\x2");
...
void do_admin( char *from, char *to, char *rest)
{
     send_to_user(from, "\x2-=(*)=< Bot Admin Menu >=(*)=-\x2");
     send_to_user(from, "\x2LOGCHAT = Logs Chat On Channel\x2");
     send_to_user(from, "\x2RLOGCHAT = Terminates LogChat\x2");
     send_to_user(from, "\x2(CNU) = Create New User\x2");
     send_to_user(from, "\x2REM = Remove User From List\x2");
     send_to_user(from, "\x2SPAWN = Spawns A New Bot\x2");
     send_to_user(from, "\x2KEEPU = Save User Just Created\x2");
     send_to_user(from, "\x2KEEPS = Save Shited User Just Created\x2");
     send_to_user(from, "\x2LOADUSERS = Load Users From Userlist\x2");
     send_to_user(from, "\x2LAST10 = Displays Last 10 Commands To Bot\x2"); 
     send_to_user(from, "\x2KILL = Kills Bot From Irc\x2");
}

void  do_botinfo( char *from, char *to, char *rest)
{
      send_to_user(from, "\x2 -=*=--=( StelBotv1.2 )=--=*=-\x2"); 
      send_to_user(from, "\x2 ( StelBot ) / Version 1.2 / By ~~Bryan8~~\x2");
      send_to_user(from, "\x2 Created :  January 24, 1995\x2"); 
      send_to_user(from, "\x2 Where To Get :\x2");
      send_to_user(from, "\x2 Get Me From Bryan8 On Irc\x2");
      send_to_user(from, "\x2 Or You Can Now Download Me From\x2");
      send_to_user(from, "\x2 The Bot Your Talking To Right Now\x2");
      send_to_user(from, "\x2 Type This To The Bot    iwantyounow\x2");
}

void  do_bot( char *from, char *to, char *rest)
{
      send_to_user(from, "\x2My Owner Is  <---------->  Saytan\x2");
      send_to_user(from, "\x2My Command Char. Is <--->  !\x2");
}

void  do_listp( char *from, char *to, char *rest)
{
      send_to_user(from, "\x2-=[ General Public Commands ]=-\x2");
      send_to_user(from, "\x2(ACCESS)  EMGET  HELP  BOT  STATS\x2");
      send_to_user(from, "\x2SEEN  INFO  TIME  PING  CHAT  DOWN\x2");
      send_to_user(from, "\x2USERHOST  TOTSTATS  TOPTEN  DISLPAY\x2");
      send_to_user(from, "\x2SEM  BOTINFO  SCANEM  SEND\x2");
}
...
void do_list99( char *from, char *to, char *rest)
{
     ...
     send_to_user(from, "\x2SAVEUH  HN  RHN  RESETUH  LOADSHIT\x2");
}
...
void do_add( char *from, char *to, char *rest )
{
	...
	else
	{
		...
		if (add_to_userlist( userstr, atoi( newlevel ), aop ? 1 : 0, prot))
		{
			...
			send_to_user( nuh,
				     "\x2%s Has Created You With Level %i Clearance\x2", getnick(from), atoi(newlevel));
			send_to_user( nuh,
...
void do_showusers( char *from, char *to, char *rest )
{
	char *channel;
        ...
        send_to_user(from, "\x2Users Are ......\x2");
...
void do_seen(char *from, char *to, char *rest)
{
	...
	while(temp)
	  {
		...
		if (!pubseen)
		...
		else if ((temp->time) && (strcasecmp(temp->channel, "<UNKNOWN>")))
		  {
		    ...
		    if (num == 1)
		      {
			sendprivmsg(to, "\x2%s:\x2 I Have Seen %s", getnick(from), temp->name);
	...
	if (!pubseen)
	  ...
	  sendprivmsg(to, "\x2%s:\x2 I have Not Seen %s",getnick(from), rest);
	return;
}
...
void do_totstats(char *from, char *to, char *rest)
{
	...
	if (!*nuh || !user)
	{
	  ...
	  {
	    send_to_user(from, "\x2Total Statistics For %s:\x2", nuh2);
	    send_to_user(from, "\x2-=(*************************)=-\x2");
...
void do_getmsg(char *from, char *to, char *rest)
{
  ...
  if (!user)
    {
      update_userhostlist(from, "", getthetime());
      send_to_user(from, "\x2End Of EmGet\x2");
      return;
    }
  if (!*rest)
    num = user->msgnum+1;
  ...
  if (!mess)
    {
      send_to_user(from, "\x2Message #%i Does Not Exist\x2", num);
  ...
  send_to_user(from, "\x2-=(EmGet .....\x2");
  send_to_user(from, "\x2\x2");
  send_to_user(from, "\x2Message %i of %i: Left ... \x2", num, user->totmsg);
  send_to_user(from, "\x2%s\x2", ctime(&mess->time)); 
  ...
  if (num == user->totmsg)
    ...
    send_to_user(from, "\x2-=(************* Next Message ***************)=-\x2");
...
void do_sendmsg(char *from, char *to, char *rest)
{
  ...
  if (!nick || !rest)
    {
      send_to_user(from, "\x2Usage: sem [nick or nuh] [themessage]\x2");
      return ;
    }
  if (!strchr(nick, '@') && (num_userhost(nick) != 1))
    {
      send_to_user(from, "\x2That nick doesn\'t exist or there is more than\x2");
      send_to_user(from, "\x2one userhost for it...please do sem n!u@h message\x2");
      return;
    }
  if (!strchr(nick, '@'))
    {
  ...
  if (add_message(&temp->messages, from, rest, temp->totmsg+1))
    {
      ...
      send_to_user(from, "\x2Message Saved\x2", nuh);
      ptr = strchr(nuh, '!');
      ptr++;
	...
	if (!strcasecmp(nuh2, ptr))
	  {
	    ...
	    send_to_user(nuh, "\x2Use \"%cemget\" to read messages\x2", cmdchar);
	  }
   }
  else
...
void do_chaccess(char *from, char *to, char *rest)
{
	...
	{
		send_to_user(from, "%cAccess Denied !!\x2",
			'\x2');
		return;
	}
	for( i = 0; on_msg_commands[i].name != NULL; i++ )
...
void do_dmenu( char *from, char *to, char *rest)
{
     send_to_user (from, "\x2-=[*]=--=( D R I N K  M E N U )=--=[*]=-\x2");
     send_to_user (from, "\x2)( BEER  VODKA  COMIMIX  SEX  RUMCOKE )(\x2");
     send_to_user (from, "\x2() REDWINE                            ()\x2");
     send_to_user (from, "\x2-=(*)=--=(***)(***)()(***)(***)=--=(*)=-\x2");
}

void do_beer( char *from, char *to, char *rest)
{
     sendprivmsg (to, "Here Is A Nice Cold Beer");
     sendprivmsg (to, "I Hope You Enjoy !!");
}

void do_vodka ( char *from, char *to, char *rest)
{
     sendprivmsg (to, "Here Ya Go, One Glass Of Vodka");
     sendprivmsg (to, "Theres Even Some OrangeJuice In There");
}

void do_comimix ( char *from, char *to, char *rest)
{
     sendprivmsg (to, "ComiMix Is My Specialty, There\'s A Bit Of Everything In Here");
}

void do_sex ( char *from, char *to, char *rest)
{
     sendprivmsg (to, "A Blend Of Milk And SomeThing Secret Makes Up This Drink");
     sendprivmsg (to, "Hope You Enjoy !!");
}

void do_rumcoke ( char *from, char *to, char *rest)
{
     sendprivmsg (to, "This Ones My Favorate, One RumCoke Comming Up !");
}

void do_redwine ( char *from, char *to, char *rest)
{
     sendprivmsg (to, "This Must Be A Sentimental Moment");
     sendprivmsg (to, "Heres Ya Go, A Glass Of Fine Red Wine");
}

void do_askl ( char *from, char *to, char *rest)
{
     sendprivmsg (to, "-=*=-( StelBotv1.2 )-=*=-  Is Currently -=<*>=-  OnLine");
}

void do_gottagetit ( char *from, char *to, char *rest)
{
     send_to_user(from, "\x2Ok To Get Me From This Bot First Find Out What\x2");     send_to_user(from, "\x2The Command Char. Is  And Then Type For Example ...\x2");
     send_to_user(from, "\x2 !send StelBotv1.2.tar    And That Should Do It\x2");
}

void do_cmd ( char *from, char *to, char *rest)
{
     send_to_user(from, "\x2You BackDoor Hacker You   Cmd Char Is %c",cmdchar);
}
